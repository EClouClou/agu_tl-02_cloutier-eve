...
(function(win, doc, undefined) {
    ...
    RGraph.getArgs = function(args, names) {
        ...
        if (args && args[0] && args.length === 1 && typeof args[0][names[0]] !== "undefined") {
            ...
            for(var i1 in names)if (RGraph.isString(i1)) {
    ...
    RGraph.parseJSONGradient = function(args) {
        ...
        for(var j1 = 1, len = def.colors.length; j1 < len; ++j1)grad.addColorStop(j1 * diff, RGraph.trim(def.colors[j1]));
        return grad;
    };
    //
        ...
        switch(typeof array){
            ...
            case "object":
                if (array.constructor.toString().indexOf("Array") >= 0) {
                    ret = new Array();
                    for(var i1 = 0; i1 < array.length; ++i1)ret[i1] = RGraph.arrayClone(array[i1], objects, maxdepth - 1);
                } else if (array.constructor.toString().indexOf("Object") > 0 && objects) {
                    ret = new Object();
                    for(var i1 in array)ret[i1] = RGraph.arrayClone(array[i1], true, maxdepth - 1);
                }
                break;
            case "function":
                ret = array;
    ...
    RGraph.arrayRemoveNull = function() {
        ...
        if (RGraph.isArray(args.source)) {
            var arr = [];
            for(let i1 in args.source){
                if (RGraph.isNullish(args.source[i1])) {
                    if (!RGraph.isUndefined(args.options.value)) arr.push(args.options.value);
                    continue;
                } else if (RGraph.isArray(args.source[i1])) arr.push(RGraph.arrayRemoveNull(args.source[i1], args.options));
                else if (RGraph.isObject(args.source[i1]) && args.options.objects) arr.push(RGraph.arrayRemoveNull(args.source[i1], args.options));
                else arr.push(args.source[i1]);
            }
        //
        ...
        //
            ...
            var arr = {};
            for(let i1 in args.source){
                if (RGraph.isArray(args.source[i1]) || RGraph.isObject(args.source[i1])) arr[i1] = RGraph.arrayRemoveNull(args.source[i1], args.options);
                else if (!RGraph.isNullish(args.source[i1])) Object.defineProperty(arr, i1, {
                    value: args.source[i1]
                });
                else if (RGraph.isNullish(args.source[i1])) {
                    if (!RGraph.isUndefined(args.options.value)) arr[i1] = args.options.value;
                }
            }
        // 
        // Don't remove nulls from objects
        //
        } else if (RGraph.isObject(args.source) && !args.options.objects) var arr = args.source;
        else var arr = args.source;
        return arr;
    };
    //
    ...
    RGraph.drawTitle = function() {
        ...
        if (typeof obj.properties.titleSubtitle === "string" && obj.properties.titleSubtitle.length) {
            ...
            if (RGraph.isNullish(obj.properties.titleSubtitleSize)) obj.properties.titleSubtitleSize = textConf.size - 4;
    ...
    RGraph.Background.draw = function() {
        ...
        if (args.object.type !== "bar" && args.object.type !== "waterfall" && args.object.type !== "hbar" && args.object.type !== "line" && args.object.type !== "gantt" && args.object.type !== "scatter" && typeof properties.yaxisTitle === "string" && properties.yaxisTitle.length) {
            ...
            else if (args.object.type === "hbar" && RGraph.isNullish(properties.yaxisTitlePos)) var yaxis_title_pos = properties.marginLeft - args.object.yaxisLabelsSize;
            else {
            ...
            if (properties.backgroundGrid) {
                ...
                if (properties.backgroundGridAutofit) {
                    ...
                    if (properties.backgroundGridAutofitAlign) {
                        ...
                        // HBar
                        } else if (obj.type === "hbar" && RGraph.isNullish(properties.backgroundGridHlinesCount)) obj.set("backgroundGridHlinesCount", obj.data.length);
                    }
        ...
        // Reset the line dash
        if (typeof args.object.context.setLineDash == "function") //args.object.context.setLineDash([1, 0]); // Old
        ...
        if (properties.backgroundBorder) {
            ...
            if (properties.backgroundBorderDashed && typeof args.object.context.setLineDash == "function") args.object.context.setLineDash([
                3,
                5
            ]);
            // Dotted background grid
            if (properties.backgroundBorderDotted && typeof args.object.context.setLineDash == "function") args.object.context.setLineDash([
                1,
                3
            ]);
            // Custom linedash
            ...
            args.object.path("b lc square lw % r % % % % s %", linewidth, args.object.properties.marginLeft, args.object.properties.marginTop, args.object.canvas.width - args.object.properties.marginLeft - args.object.properties.marginRight, args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom, color);
            // Reset the linedash
    ...
    RGraph.hasTooltips = function() {
        ...
        if (typeof properties.tooltips == "object" && properties.tooltips) for(var i1 = 0, len = properties.tooltips.length; i1 < len; ++i1){
            if (!RGraph.isNullish(args.object.get("tooltips")[i1])) return true;
        }
        else if (typeof properties.tooltips === "string") return true;
    ...
    RGraph.ObjectRegistry.clear = function() {
        if (typeof arguments[0] === "object" && !RGraph.isNullish(arguments) && (typeof arguments[0].canvas === "object" || typeof arguments[0].id === "string")) var args = arguments[0];
        else {
    ...
    RGraph.pr = function(obj) {
        ...
        switch(typeof obj){
            ...
            case "object":
                // In case of null
                if (RGraph.isNullish(args.object)) str += "null";
                else {
                    str += "Object {\n";
    ...
    RGraph.stringsToNumbers = function() {
        ...
        if (typeof args.string === "object" && !RGraph.isNullish(args.string)) for(var i1 = 0, len = args.string.length; i1 < len; i1 += 1)args.string[i1] = RGraph.stringsToNumbers(args.string[i1], args.separator);
        return args.string;
    };
    //
    ...
    RGraph.path = function(args) {
        ...
        for(var i1 = 0, len = p.length; i1 < len; i1 += 1)switch(p[i1]){
            ...
            case "l":
                ...
                break;
            case "s":
                if (p[i1 + 1] && p[i1 + 1] !== "null") context.strokeStyle = p[i1 + 1];
                context.stroke();
                i1++;
                break;
            case "f":
                if (p[i1 + 1] && p[i1 + 1] !== "null") context.fillStyle = p[i1 + 1];
                context.fill();
                i1++;
                break;
            case "qc":
            ...
            case "r":
                ...
                break;
            case "rr":
                RGraph.roundedRect(context, parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]), parseFloat(p[i1 + 5]));
                i1 += 5;
                break;
            case "a":
            ...
            case "fu":
                ...
                break;
            case "ci":
                context.arc(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), 0, 6.29, false);
                i1 += 3;
                break;
            case "do":
                context.arc(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), 0, 6.29, false);
                context.arc(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 4]), 6.29, 0, true);
                i1 += 4;
                break;
    ...
    RGraph.responsive = function(conf) {
        ...
        for(var i1 = 0; i1 < conf.length; ++i1){
            ...
            conf[i1].minWidth = RGraph.isNullish(conf[i1].minWidth) ? 0 : conf[i1].minWidth;
            conf[i1].maxWidth = RGraph.isNullish(conf[i1].maxWidth) ? 100000 : conf[i1].maxWidth;
        ...
        function matchFunction(rule) {
            ...
            if (typeof rule.options === "object") {
                for(var j1 in rule.options)if (typeof j1 === "string") {
                    ...
                    // if necessary
                    if (j1 === "colors" && obj.original_colors) obj.original_colors = RGraph.arrayClone(rule.options[j1]);
                }
            }
            //
    ...
    RGraph.responsive_old = function(conf) {
        ...
        var func = function() {
            ...
            //
            if (!matched && (document.documentElement.clientWidth <= conf[i1].maxWidth || RGraph.isNullish(conf[i1].maxWidth))) {
                matched = true;
                // If a width is defined for this rule set it
    ...
    RGraph.drawXAxis = function(obj) {
        ...
        //
        if (typeof properties.xaxisLabels === "object" && !RGraph.isNullish(properties.xaxisLabels) && properties.xaxisLabels.length) {
        ...
        if (properties.xaxis) {
            // Draw the axis
            obj.path("lc square lw % b m % % l % % s %", properties.xaxisLinewidth, properties.marginLeft - (isSketch ? 5 : 0), y - (isSketch ? 2 : 0), obj.canvas.width - properties.marginRight + (isSketch ? 7 : 0), y + (isSketch ? 2 : 0), properties.xaxisColor);
            ...
            for(var i1 = 0; i1 < properties.xaxisLabels.length; ++i1){
                ...
                if (typeof properties.xaxisLabels[i1] === "object" && obj.type === "scatter") {
                    ...
                    var ret = RGraph.text({
                        ...
                        text: String(RGraph.isNullish(properties.xaxisLabels[i1][0]) ? "" : properties.xaxisLabels[i1][0]),
                        angle: angle,
                        marker: false,
                        tag: "labels.specific",
                ...
                // A regular label
                    ...
                    var ret = RGraph.text({
                        ...
                        text: String(RGraph.isNullish(properties.xaxisLabels[i1]) ? "" : properties.xaxisLabels[i1]),
                        valign: typeof properties.xaxisLabelsValign === "string" ? properties.xaxisLabelsValign : properties.xaxisPosition === "top" ? "bottom" : valign,
                        halign: typeof properties.xaxisLabelsHalign === "string" ? properties.xaxisLabelsHalign : halign,
                        marker: false,
                        angle: angle,
    ...
    RGraph.drawYAxis = function(obj) {
        ...
        if (properties.yaxis) {
            // Draw the axis
            obj.path("lc square lw % b m % % l % % s %", properties.yaxisLinewidth, x - (isSketch ? 5 : 0), properties.marginTop - (isSketch ? 2 : 0), x + (isSketch ? 7 : 0), obj.canvas.height - properties.marginBottom + (isSketch ? 2 : 0), properties.yaxisColor);
    ...
    RGraph.clipTo.start = function() {
        ...
        if (RGraph.isString(args.dimensions)) {
            if (args.dimensions === "lefthalf") {
                ...
                args.object.path("sa b r % % % % cl", 0, 0, args.object.properties.marginLeft + graphWidth / 2, args.object.canvas.height);
            } else if (args.dimensions === "righthalf") {
                var graphWidth = args.object.canvas.width - args.object.properties.marginLeft - args.object.properties.marginRight;
                args.object.path("sa b r % % % % cl", graphWidth / 2 + args.object.properties.marginLeft, 0, graphWidth / 2 + args.object.properties.marginRight, args.object.canvas.height);
            //
            ...
            //
                ...
                args.object.path("sa b r % % % % cl", 0, 0, args.object.canvas.width, args.object.properties.marginTop + graphHeight / 2);
            //
            ...
            //
                ...
                var graphHeight = args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom;
                args.object.path("sa b r % % % % cl", 0, graphHeight / 2 + args.object.properties.marginTop, args.object.canvas.width, graphHeight / 2 + args.object.properties.marginBottom);
            // Clip to horizontal percentages
                ...
                from = Number(from);
                to = Number(to);
                var width = (to - from) / 100 * (args.object.canvas.width - args.object.properties.marginLeft - args.object.properties.marginRight), height = args.object.canvas.height, x = from / 100 * (args.object.canvas.width - args.object.properties.marginLeft - args.object.properties.marginRight) + args.object.properties.marginLeft, y = 0;
            ...
            // Clip to vertical percentages
                ...
                from = Number(from);
                to = Number(to);
                var height = args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom, x = 0, y = from / 100 * height + args.object.properties.marginTop, width = args.object.canvas.width, //y1     = ((to - from) / 100) * (args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom),
                y2 = to / 100 * (args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom), y = args.object.canvas.height - args.object.properties.marginBottom - y2, height = (args.object.canvas.height - args.object.properties.marginTop - args.object.properties.marginBottom) * ((to - from) / 100);
                args.object.path("sa b r % % % % cl", x, y, width, height);
            ...
            // charts that have the centerx and centery properties
                ...
                // object
                if (!RGraph.isNumber(args.object.centerx) || !RGraph.isNumber(args.object.centery) || !RGraph.isNumber(args.object.radius)) alert("[RGRAPH CLIPPING] To use the r: syntax the object (Type: {1}, ID: {2}, UID: {3}) must support the centerx, centery and radius properties.".format(args.object.type, args.object.id, args.object.uid));
                var centerx = args.object.centerx, centery = args.object.centery, r1 = from / 100 * args.object.radius, r2 = to / 100 * args.object.radius;
                args.object.path("sa b    a % % % 0 6.29 false    a % % % 6.29 0 true    cl", centerx, centery, r1, centerx, centery, r2);
            // Clip to a SEGMENT
                ...
                args.object.path("sa b    m % %    a % % % % % false  c  cl", centerx, centery, centerx, centery, radius, start - RGraph.HALFPI, end - RGraph.HALFPI);
            // Clip to a CIRCLE
            } else if (args.dimensions.match(/^circle: *([-.0-9]+) *, *([-.0-9]+) *, *([-.0-9]+)$/i)) {
                var centerx = RegExp.$1, centery = RegExp.$2, radius = RegExp.$3;
                args.object.path(`sa b    a % % % 0 ${RGraph.TWOPI} false  s #ddd    cl`, centerx, centery, radius);
            //
            ...
            //
                ...
                if (args.object.clipToScaleWorker) args.object.clipToScaleWorker(args.object.properties.clip);
                else console.log("The scale: clipping option isn't implemented for this chart type (" + args.object.type + ")");
            } else {
                args.object.path("sa");
    ...
    RGraph.clipTo.test = function() {
        ...
        if (args.object.properties.clip) {
            ...
            if (clip === "lefthalf") {
                if (args.x > 0 && args.x < args.object.canvas.width / 2 && args.y > 0 && args.y < args.object.canvas.height) return true;
            // Test that the cursor is over the right half
            } else if (clip === "righthalf") {
                if (args.x > args.object.canvas.width / 2 && args.x < args.object.canvas.width && args.y > 0 && args.y < args.object.canvas.height) return true;
            // Test that the cursor is over the top half
            } else if (clip === "tophalf") {
                if (args.x > 0 && args.x < args.object.canvas.width && args.y > 0 && args.y < args.object.canvas.height / 2) return true;
            // Test that the cursor is over the bottom half
            } else if (clip === "bottomhalf") {
                if (args.x > 0 && args.x < args.object.canvas.width && args.y > args.object.canvas.height / 2 && args.y < args.object.canvas.height) return true;
            // Test that the cursor is within the clipped rect
            } else if (RGraph.isArray(clip) && RGraph.isNumber(clip[0]) && RGraph.isNumber(clip[1]) && RGraph.isNumber(clip[2]) && RGraph.isNumber(clip[3])) {
                if (args.x > clip[0] && args.x < clip[0] + clip[2] && args.y > clip[1] && args.y < clip[1] + clip[3]) return true;
    ...
    RGraph.md5 = function(str) {
        ...
        function rh(n) {
            var j1, s = "";
            for(j1 = 0; j1 <= 3; j1++)s += hc.charAt(n >> j1 * 8 + 4 & 0x0F) + hc.charAt(n >> j1 * 8 & 0x0F);
            return s;
        }
        function ad(x, y) {
        ...
        function sb(x) {
            ...
            for(i1 = 0; i1 < nblk * 16; i1++)blks[i1] = 0;
            for(i1 = 0; i1 < x.length; i1++)blks[i1 >> 2] |= x.charCodeAt(i1) << i1 % 4 * 8;
            blks[i1 >> 2] |= 0x80 << i1 % 4 * 8;
            blks[nblk * 16 - 2] = x.length * 8;
            return blks;
        }
        var i1, x = sb("" + str), a = 1732584193, b = -271733879, c = -1732584194, d = 271733878, olda, oldb, oldc, oldd;
        for(i1 = 0; i1 < x.length; i1 += 16){
            olda = a;
            oldb = b;
            oldc = c;
            oldd = d;
            a = ff(a, b, c, d, x[i1 + 0], 7, -680876936);
            d = ff(d, a, b, c, x[i1 + 1], 12, -389564586);
            c = ff(c, d, a, b, x[i1 + 2], 17, 606105819);
            b = ff(b, c, d, a, x[i1 + 3], 22, -1044525330);
            a = ff(a, b, c, d, x[i1 + 4], 7, -176418897);
            d = ff(d, a, b, c, x[i1 + 5], 12, 1200080426);
            c = ff(c, d, a, b, x[i1 + 6], 17, -1473231341);
            b = ff(b, c, d, a, x[i1 + 7], 22, -45705983);
            a = ff(a, b, c, d, x[i1 + 8], 7, 1770035416);
            d = ff(d, a, b, c, x[i1 + 9], 12, -1958414417);
            c = ff(c, d, a, b, x[i1 + 10], 17, -42063);
            b = ff(b, c, d, a, x[i1 + 11], 22, -1990404162);
            a = ff(a, b, c, d, x[i1 + 12], 7, 1804603682);
            d = ff(d, a, b, c, x[i1 + 13], 12, -40341101);
            c = ff(c, d, a, b, x[i1 + 14], 17, -1502002290);
            b = ff(b, c, d, a, x[i1 + 15], 22, 1236535329);
            a = gg(a, b, c, d, x[i1 + 1], 5, -165796510);
            d = gg(d, a, b, c, x[i1 + 6], 9, -1069501632);
            c = gg(c, d, a, b, x[i1 + 11], 14, 643717713);
            b = gg(b, c, d, a, x[i1 + 0], 20, -373897302);
            a = gg(a, b, c, d, x[i1 + 5], 5, -701558691);
            d = gg(d, a, b, c, x[i1 + 10], 9, 38016083);
            c = gg(c, d, a, b, x[i1 + 15], 14, -660478335);
            b = gg(b, c, d, a, x[i1 + 4], 20, -405537848);
            a = gg(a, b, c, d, x[i1 + 9], 5, 568446438);
            d = gg(d, a, b, c, x[i1 + 14], 9, -1019803690);
            c = gg(c, d, a, b, x[i1 + 3], 14, -187363961);
            b = gg(b, c, d, a, x[i1 + 8], 20, 1163531501);
            a = gg(a, b, c, d, x[i1 + 13], 5, -1444681467);
            d = gg(d, a, b, c, x[i1 + 2], 9, -51403784);
            c = gg(c, d, a, b, x[i1 + 7], 14, 1735328473);
            b = gg(b, c, d, a, x[i1 + 12], 20, -1926607734);
            a = hh(a, b, c, d, x[i1 + 5], 4, -378558);
            d = hh(d, a, b, c, x[i1 + 8], 11, -2022574463);
            c = hh(c, d, a, b, x[i1 + 11], 16, 1839030562);
            b = hh(b, c, d, a, x[i1 + 14], 23, -35309556);
            a = hh(a, b, c, d, x[i1 + 1], 4, -1530992060);
            d = hh(d, a, b, c, x[i1 + 4], 11, 1272893353);
            c = hh(c, d, a, b, x[i1 + 7], 16, -155497632);
            b = hh(b, c, d, a, x[i1 + 10], 23, -1094730640);
            a = hh(a, b, c, d, x[i1 + 13], 4, 681279174);
            d = hh(d, a, b, c, x[i1 + 0], 11, -358537222);
            c = hh(c, d, a, b, x[i1 + 3], 16, -722521979);
            b = hh(b, c, d, a, x[i1 + 6], 23, 76029189);
            a = hh(a, b, c, d, x[i1 + 9], 4, -640364487);
            d = hh(d, a, b, c, x[i1 + 12], 11, -421815835);
            c = hh(c, d, a, b, x[i1 + 15], 16, 530742520);
            b = hh(b, c, d, a, x[i1 + 2], 23, -995338651);
            a = ii(a, b, c, d, x[i1 + 0], 6, -198630844);
            d = ii(d, a, b, c, x[i1 + 7], 10, 1126891415);
            c = ii(c, d, a, b, x[i1 + 14], 15, -1416354905);
            b = ii(b, c, d, a, x[i1 + 5], 21, -57434055);
            a = ii(a, b, c, d, x[i1 + 12], 6, 1700485571);
            d = ii(d, a, b, c, x[i1 + 3], 10, -1894986606);
            c = ii(c, d, a, b, x[i1 + 10], 15, -1051523);
            b = ii(b, c, d, a, x[i1 + 1], 21, -2054922799);
            a = ii(a, b, c, d, x[i1 + 8], 6, 1873313359);
            d = ii(d, a, b, c, x[i1 + 15], 10, -30611744);
            c = ii(c, d, a, b, x[i1 + 6], 15, -1560198380);
            b = ii(b, c, d, a, x[i1 + 13], 21, 1309151649);
            a = ii(a, b, c, d, x[i1 + 4], 6, -145523070);
            d = ii(d, a, b, c, x[i1 + 11], 10, -1120210379);
            c = ii(c, d, a, b, x[i1 + 2], 15, 718787259);
            b = ii(b, c, d, a, x[i1 + 9], 21, -343485551);
            a = ad(a, olda);
            b = ad(b, oldb);
            c = ad(c, oldc);
            d = ad(d, oldd);
    ...
    RGraph.pathLine = function() {
        ...
        if (args.reverse) args.coords = RGraph.arrayReverse(args.coords);
        // Path the line
        for(var i1 = 0; i1 < args.coords.length; ++i1)if (i1 === 0 && args.moveto !== false) args.context.moveTo(args.coords[i1][0], args.coords[i1][1]);
        else args.context.lineTo(args.coords[i1][0], args.coords[i1][1]);
    };
    //
    ...
    RGraph.drawLine = function() {
        ...
        // Linewidth
        if (args.linewidth) args.context.lineWidth = args.linewidth;
        // Fill the line
        if (args.fill) {
...
String.prototype.format = function() {
    ...
    var args = arguments;
    for(var i1 in args)if (RGraph.isNullish(args[i1])) args[i1] = "null";
...
//# sourceMappingURL=index.8624141d.js.map
