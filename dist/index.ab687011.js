...
(function(win, doc, undefined) {
    ...
    RGraph.installWindowMousedownListener = function(obj) {
        if (!RGraph.window_mousedown_event_listener) {
            RGraph.window_mousedown_event_listener = function(e) {
                ...
                if (RGraph.hideTooltip && tooltip && tooltip.__object__.get("tooltipsEffect") !== "slide") {
        ...
        if (!RGraph.window_mouseup_event_listener) {
            RGraph.window_mouseup_event_listener = function(e) {
                ...
                if (RGraph.Registry.get("adjusting") || RGraph.Registry.get("adjusting.gantt")) {
                    ...
                    if (obj && obj.type === "line") obj.data_arr = RGraph.arrayLinearize(obj.data);
                ...
                for(var i1 = 0; i1 < tags.length; ++i1){
                    if (tags[i1].__object__ && tags[i1].__object__.isRGraph) {
                        if (!tags[i1].__object__.get("annotatable")) {
                            if (!tags[i1].__rgraph_trace_cover__ && !noredraw) RGraph.clear(tags[i1]);
                            else var noredraw = true;
                        }
                    }
                }
    ...
    RGraph.installCanvasMouseupListener = function(obj) {
        if (!obj.canvas.rgraph_mouseup_event_listener) {
            obj.canvas.rgraph_mouseup_event_listener = function(e) {
                ...
                if (objects) for(var i1 = 0, len = objects.length; i1 < len; i1 += 1){
                    var obj = objects[i1], id = objects[i1].id;
                    // =========================================================================
        ...
        if (!obj.canvas.rgraph_mousemove_event_listener) {
            obj.canvas.rgraph_mousemove_event_listener = function(e) {
                ...
                if (objects && objects.length > 0) {
                    for(var i1 = 0, len = objects.length; i1 < len; i1 += 1){
                        var obj = objects[i1];
                        var id = obj.id;
                        // Record the uid
                        uids[obj.uid] = true;
                        ...
                        if (obj && obj.properties.highlightDataset && obj.properties.highlightDatasetEvent === "mousemove") {
                            ...
                            if (shape && !excludedNumber && !excludedArray) // Store the index that's highlighted on the canvas if
                            // it's different to what's previously been stored.
                            {
                                if (shape.dataset !== obj.canvas.rgraph_highlightDataset_dataset) {
                                    ...
                                    if (RGraph.isFunction(obj.properties.highlightDatasetCallback)) obj.properties.highlightDatasetCallback({
                                        dataset: shape.dataset,
                                        object: obj
                                    });
                                }
                        ...
                        if (obj.coords && obj.coords.key && obj.coords.key.length) {
                            ...
                            for(var i1 = 0, overkey = false; i1 < obj.coords.key.length; ++i1){
                                if (mouseXY[0] >= obj.coords.key[i1][0] && mouseXY[0] <= obj.coords.key[i1][0] + obj.coords.key[i1][2] && mouseXY[1] >= obj.coords.key[i1][1] && mouseXY[1] <= obj.coords.key[i1][1] + obj.coords.key[i1][3]) {
                                    RGraph.Registry.set("key-element", obj.coords.key[i1]);
                                    overkey = true;
                                }
                        ...
                        // ================================================================================================ //
                        var func = null;
                        if (!func && typeof obj.onmousemove == "function") var func = obj.onmousemove;
                        //
                        // 
                        //
                        if (shape) {
                            var index = shape.sequentialIndex;
                            if (typeof obj["$" + index] === "object" && typeof obj["$" + index].onmousemove == "function") var func2 = obj["$" + index].onmousemove;
                        }
                        //
                        ...
                        if (shape && (typeof func == "function" || typeof func2 == "function" || typeof obj.get("link") === "string")) {
                            if (obj.get("eventsMousemoveRevertto") == null) obj.set("eventsMousemoveRevertto", e.target.style.cursor);
                            ...
                            // event listeners
                            if (typeof RGraph.events === "object" && typeof RGraph.events[obj.uid] === "object") {
                                for(i1 in RGraph.events[obj.uid])if (typeof i1 === "string" && typeof RGraph.events[obj.uid][i1] === "object" && RGraph.events[obj.uid][i1][1] === "onmousemove" && typeof RGraph.events[obj.uid][i1][2] === "function") RGraph.events[obj.uid][i1][2](obj);
                            }
                        //return;
                        ...
                        // ======================================================
                        var func = null;
                        if (!func && typeof obj.onmouseover === "function") func = obj.onmouseover;
                        // Allow for individually index functions to be specified
                        if (shape) {
                            var index = shape.sequentialIndex;
                            if (typeof obj["$" + index] == "object" && typeof obj["$" + index].onmouseover == "function") var func2 = obj["$" + index].onmouseover;
                        } else {
                        ...
                        if (shape) {
                            if (obj.__mouseover_shape_index__ === shape.index === false) {
                                ...
                                // event listeners
                                if (typeof RGraph.events === "object" && typeof RGraph.events[obj.uid] === "object") {
                                    for(i1 in RGraph.events[obj.uid])if (typeof i1 === "string" && typeof RGraph.events[obj.uid][i1] === "object" && RGraph.events[obj.uid][i1][1] === "onmouseover" && typeof RGraph.events[obj.uid][i1][2] === "function") RGraph.events[obj.uid][i1][2](obj);
                                }
                            }
                        } else {
                            obj.__mouseover_shape_index__ = null;
                        ...
                        if (shape && (tooltips && tooltips[shape.index] || shape.tooltip) && tooltips_event.indexOf("mousemove") !== -1 && (!current_tooltip // Is there a tooltip being shown?
                    ...
                    for(var i1 = 0; i1 < objects.length; ++i1)if (!uids[objects[i1].uid]) objects[i1].__mouseover_shape_index__ = null;
                } else {
                    // Reset the mouseover flag on all of this canvas tags objects
                    var objects = RGraph.OR.getObjectsByCanvasID(e.target.id);
                    for(var i1 = 0; i1 < objects.length; i1++){
                        if (typeof objects[i1].__mouseover_shape_index__ === "number") RGraph.fireCustomEvent(objects[i1], "onmouseout");
                        objects[i1].__mouseover_shape_index__ = null;
                    }
                    RGraph.__mouseover_objects__ = [];
                    RGraph.last_mouseover_uid = null;
                    RGraph.last_mouseover_object = null;
                        ...
                        if (obj && obj.isRGraph) {
                            ...
                            switch(obj.type){
                                ...
                                case "gantt":
                                    var shape = obj.getShape(e);
                                    if (shape) {
                                        var data = typeof obj.data[shape.dataset] === "object" && obj.data[shape.dataset][shape.index] && obj.data[shape.dataset][shape.index].start ? obj.data[shape.dataset][shape.index] : obj.data[shape.dataset];
                                        var mouseXY = RGraph.getMouseXY(e);
                            ...
                            //
                            if (RGraph.isNullish(obj.properties.adjustableOnly) || typeof obj.properties.adjustableOnly === "undefined" || RGraph.isArray(obj.properties.adjustableOnly) && obj.isAdjustable && obj.isAdjustable(shape)) {
        ...
        if (!obj.canvas.rgraph_click_event_listener) {
            obj.canvas.rgraph_click_event_listener = function(e) {
                ...
                for(var i1 = 0, len = objects.length; i1 < len; i1 += 1){
                    ...
                    //
                    var func = null;
                    if (!func && typeof obj.onclick == "function") func = obj.onclick;
                    if (shape && typeof func == "function") {
                        func(e, shape);
                        // Go through the RGraph.events array looking for more
                        // event listeners
                        if (typeof RGraph.events === "object" && typeof RGraph.events[obj.uid] === "object") {
                            for(i1 in RGraph.events[obj.uid])if (typeof i1 === "string" && typeof RGraph.events[obj.uid][i1] === "object" && RGraph.events[obj.uid][i1][1] === "onclick" && typeof RGraph.events[obj.uid][i1][2] === "function") RGraph.events[obj.uid][i1][2](obj);
                        }
                        //
                    ...
                    if (key) RGraph.fireCustomEvent(obj, "onkeyclick");
                    //
    ...
    RGraph.evaluateCursor = function(e) {
        ...
        for(var i1 = 0, len = objects.length; i1 < len; i1 += 1)if (objects[i1].getShape && objects[i1].getShape(e) || objects[i1].overChartArea && objects[i1].overChartArea(e)) {
            var obj = objects[i1];
            var id = obj.id;
        }
        if (!RGraph.isNullish(obj)) {
            if (obj.getShape && obj.getShape(e)) {
                var shape = obj.getShape(e);
                if (!obj.isMarimekko && obj.get("tooltips") || obj.isMarimekko && obj.get("marimekkoTooltips")) {
                    var text = RGraph.parseTooltipText(obj.isMarimekko ? obj.get("marimekkoTooltips") : obj.get("tooltips"), shape.sequentialIndex);
                    if (!text && shape.object.type == "scatter" && shape.index) text = RGraph.parseTooltipText(obj.get("tooltips"), shape.index);
                    //
            ...
            if (!RGraph.isNullish(obj) && obj.get("keyInteractive")) {
                for(var j = 0; j < obj.coords.key.length; ++j)if (mouseX > obj.coords.key[j][0] && mouseX < obj.coords.key[j][0] + obj.coords.key[j][2] && mouseY > obj.coords.key[j][1] && mouseY < obj.coords.key[j][1] + obj.coords.key[j][3]) var pointer = true;
            }
        }
        //
        ...
        for(var i1 = 0, len = objects.length; i1 < len; i1 += 1)if (objects[i1] && objects[i1][1].get("resizable")) var resizable = true;
        if (resizable && mouseX > e.target.width - 32 && mouseY > e.target.height - 16) pointer = true;
        if (pointer) e.target.style.cursor = "pointer";
        else if (e.target.style.cursor == "pointer") e.target.style.cursor = "default";
        ...
        else if (e.target.style.cursor === "move") e.target.style.cursor = "default";
        // =========================================================================
        // Line chart dataset highlighting
        // =========================================================================
        var objects = RGraph.OR.getObjectsByXY(e);
        for(let i1 = 0; i1 < objects.length; ++i1){
            if (objects[i1] && objects[i1].type && objects[i1].type === "line" && objects[i1].properties.highlightDataset) {
                var [x, y] = RGraph.getMouseXY(e);
                var shape = objects[i1].over(x, y);
                var excludedNumber = RGraph.isNumber(objects[i1].properties.highlightDatasetExclude) && shape && objects[i1].properties.highlightDatasetExclude === shape.dataset;
                var excludedArray = RGraph.isArray(objects[i1].properties.highlightDatasetExclude) && shape && objects[i1].properties.highlightDatasetExclude.includes(shape.dataset);
                if (shape && !excludedNumber && !excludedArray) e.target.style.cursor = "pointer";
            }
            if (objects[i1].getShape(e)) break;
        }
        // =========================================================================
        // Interactive key
        // =========================================================================
        ...
        if (obj && obj.type == "line" && obj.get("adjustable")) {
            if (obj.getShape) {
                var shape = obj.getShape(e);
                if (shape && obj.isAdjustable(shape)) e.target.style.cursor = "ns-resize";
        ...
        if (obj && obj.type == "scatter" && obj.get("adjustable")) {
            if (obj.getShape) {
                var shape = obj.getShape(e);
                if (shape && obj.isAdjustable(shape)) e.target.style.cursor = "move";
            } else e.target.style.cursor = "default";
        }
        // =========================================================================
        // Annotatable
    ...
    RGraph.parseTooltipText = function(tooltips, idx) {
        ...
        if (typeof tooltips == "function") var text = tooltips(idx);
        else if (typeof tooltips == "string") var text = tooltips;
        else if (typeof tooltips === "object" && typeof tooltips[idx] == "function") var text = tooltips[idx](idx);
        else if (typeof tooltips === "object" && (RGraph.isNullish(tooltips[idx]) || typeof tooltips[idx] === "undefined")) return null;
        else if (typeof tooltips[idx] == "string" && tooltips[idx]) var text = tooltips[idx];
        else var text = "";
        if (typeof text === "undefined") text = "";
        else if (typeof text === "null") text = "";
    ...
    RGraph.drawCrosshairs = function(e, obj) {
        var width = obj.canvas.width, height = obj.canvas.height, mouseXY = RGraph.getMouseXY(e), x = mouseXY[0], y = mouseXY[1], marginLeft = obj.marginLeft, marginRight = obj.marginRight, marginTop = obj.marginTop, marginBottom = obj.marginBottom, prop = obj.properties, properties = obj.properties;
        //
        ...
        var crosshairsSnapToScale = function(e) {
            ...
            if (obj.properties.crosshairs && obj.properties.crosshairsSnapToScale) {
                if (obj.type === "scatter") var value = obj.getYValue(e);
                else var value = obj.getValue(e);
                ...
                if (properties.crosshairsDotted) var linedash = "[1,3]";
                else if (properties.crosshairsDashed) var linedash = "[5,5]";
                else var linedash = "[1,1]";
                // Start drawing on a "fresh" canvas
                RGraph.redraw();
                // Draw a vertical line for the HBar
                if (obj.type === "hbar") {
                    // Draw the vertical line
                    if (obj.properties.crosshairsVline) obj.path("b ld % lw % m % % l % % s %", linedash, obj.properties.crosshairsLinewidth, obj.getXCoord(value), obj.marginTop, obj.getXCoord(value), obj.canvas.height - obj.marginBottom, obj.properties.crosshairsColor);
                    // Draw the horizontal line
                    if (obj.properties.crosshairsHline) {
                        var [mouseX, mouseY] = RGraph.getMouseXY(e);
                        obj.path("b ld % lw % m % % l % % s %", linedash, obj.properties.crosshairsLinewidth, obj.properties.marginLeft, mouseY, obj.canvas.width - obj.properties.marginRight, mouseY, obj.properties.crosshairsColor);
                    }
                // Draw a horizontal line for bar/line/scatter/waterfall charts
                } else {
                    if (obj.properties.crosshairsHline) obj.path("b ld % lw % m % % l % % s %", linedash, obj.properties.crosshairsLinewidth, obj.marginLeft, obj.getYCoord(value), obj.canvas.width - obj.marginRight, obj.getYCoord(value), obj.properties.crosshairsColor);
                    // Draw a vertical line
                    if (obj.properties.crosshairsVline) {
                        var [mouseX, mouseY] = RGraph.getMouseXY(e);
                        obj.path("b ld % lw % m % % l % % s %", linedash, obj.properties.crosshairsLinewidth, mouseX, obj.properties.marginTop, mouseX, obj.canvas.height - obj.properties.marginBottom, obj.properties.crosshairsColor);
                    }
                }
            }
        };
        RGraph.redrawCanvas(obj.canvas);
        if (x >= marginLeft && y >= marginTop && x <= width - marginRight && y <= height - marginBottom) {
            var linewidth = properties.crosshairsLinewidth ? properties.crosshairsLinewidth : 1;
            obj.context.lineWidth = linewidth ? linewidth : 1;
            obj.path("b ss %", properties.crosshairsColor);
            //
            ...
            if (properties.crosshairsSnap && !properties.crosshairsSnapToScale) {
                ...
                var len = null;
                if (obj.type == "line") {
                    for(var i1 = 0; i1 < obj.coords.length; ++i1){
                        var length = RGraph.getHypLength(obj.coords[i1][0], obj.coords[i1][1], x, y);
                    ...
                    for(var dataset = 0; dataset < obj.coords2.length; ++dataset){
                        for(var point = 0; point < obj.coords2[dataset].length; ++point)if (obj.coords2[dataset][point][0] == x && obj.coords2[dataset][point][1] == y) {
                            obj.canvas.__crosshairs_snap_dataset__ = dataset;
                            obj.canvas.__crosshairs_snap_point__ = point;
                        }
                    }
                } else {
                    for(var i1 = 0; i1 < obj.coords.length; ++i1)for(var j = 0; j < obj.coords[i1].length; ++j){
                        // Check the mouse X coordinate
                        var len = RGraph.getHypLength(obj.coords[i1][j][0], obj.coords[i1][j][1], x, y);
            ...
            } else if (!properties.crosshairsSnap && properties.crosshairsSnapToScale) crosshairsSnapToScale(e);
            if (!properties.crosshairsSnapToScale) {
                // Determine the linedash setting
                if (properties.crosshairsDotted) obj.context.setLineDash([
                    1,
                    3
                ]);
                else if (properties.crosshairsDashed) obj.context.setLineDash([
                    5,
                    5
                ]);
                else obj.context.setLineDash([
                    1,
                    1
                ]);
                // Draw a vertical line
            ...
            if (obj.type == "scatter" && properties.crosshairsCoords) {
                ...
                if (obj.type == "scatter" && obj.properties.xaxisPosition === "center") yCoord = (yCoord - obj.max / 2) * 2;
                ...
                if (typeof properties.crosshairsCoordsFormatterX === "function") xCoord = properties.crosshairsCoordsFormatterX({
                    object: obj,
                    value: parseInt(xCoord)
                });
                if (typeof properties.crosshairsCoordsFormatterY === "function") yCoord = properties.crosshairsCoordsFormatterY({
                    object: obj,
                    value: parseInt(yCoord)
                });
            ...
            } else if (properties.crosshairsCoords) alert("[RGRAPH] Showing crosshair coordinates is only supported on the Scatter chart");
            //
    ...
    RGraph.allowSegmentHighlight = function(opt) {
        ...
        if (!RGraph.segmentHighlightFunction) {
            RGraph.segmentHighlightFunction = function(e) {
                ...
                var r = obj.radius;
                (function() {
                    for(i = 0; i < count; i += 1)if (angle < a * (i + 1)) {
                        start = i * a;
                        end = (i + 1) * a;
                        return;
                    }
                ...
                obj.path("b m % % a % % % % % false c s % f %", obj.centerx, obj.centery, obj.centerx, obj.centery, r, start, end, stroke, fill);
            };
            obj.canvas.addEventListener("mousemove", RGraph.segmentHighlightFunction, false);
        }
    };
// End module pattern
})(window, document);

//# sourceMappingURL=index.ab687011.js.map
