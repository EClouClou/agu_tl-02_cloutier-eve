...
if (!RGraph.getArgs) RGraph.getArgs = function(args, names) {
    ...
    if (args && args[0] && args.length === 1 && typeof args[0][names[0]] !== "undefined") {
        for(var i = 0; i < names.length; ++i)if (typeof args[0][names[i]] === "undefined") args[0][names[i]] = null;
        return args[0];
    } else for(var i in names){
...
RGraph.CSV = function() {
    ...
    this.splitCSV = function(str, split) {
        ...
        if (typeof split === "object") {
            if (typeof split.char === "string") split = split.char;
            else split = ",";
        ...
        if (!preserve) for(i = 0, len = arr.length; i < len; i += 1)arr[i] = arr[i].trim();
        return arr;
    };
    //
    ...
    //
    this.fetch = function() {
        var sep = this.separator, eol = this.endofline, obj = this;
        if (this.url.substring(0, 3) === "id:" || this.url.substring(0, 4) === "str:") {
            // Get rid of any surrounding whitespace
            if (this.url.substring(0, 3) === "id:") var data = document.getElementById(this.url.substring(3)).innerHTML.trim();
            else if (this.url.substring(0, 4) === "str:") var data = this.url.substring(4).trim();
            ...
            for(var i = 0, len = obj.data.length; i < len; i += 1){
                ...
                var row = obj.splitCSV(obj.data[i], {
                    preserve: false,
                    char: sep
                });
                ...
                for(var j = 0; j < row.length; j += 1){
                    if (/^\-?[0-9.]+$/.test(row[j])) row[j] = parseFloat(row[j]);
    ...
    //
    this.row = this.getRow = function(index) {
        var row = [], start = parseInt(arguments[1]) || 0, length = arguments[2];
        // Convert a string based row name to a
        // numeric index
        if (typeof index === "string") {
            for(var i = 0; i < this.data.length; ++i)if (this.data[i][0] === index) {
                var found = true;
                index = i;
                break;
            }
        ...
        if (start < 0) row = this.data[index].slice(this.data[index].length - Math.abs(start));
        else row = this.data[index].slice(start);
        // Zero length
        if (typeof length === "number" && length === 0) row = [];
        else {
            // Positive length
            if (typeof length === "number" && length > 0) row = row.slice(0, length);
            else if (typeof length === "number" && length < 0) for(var i = 0; i < Math.abs(length); ++i)row.pop();
        }
        return row;
    };
    //
    ...
    //
    this.col = this.column = this.getColumn = this.getCol = function(index) {
        var col = [], start = arguments[1] || 0, length = arguments[2];
        // Convert a string based column name to a
        // numeric index
        if (RGraph.isString(index)) {
            for(var i = 0; i < this.data.length; ++i)if (this.data[0][i] === index) {
                var found = true;
                index = i;
                break;
            }
        ...
        if (start >= 0) {
            for(var i = start; i < this.data.length; i += 1)if (this.data[i]) col.push(this.data[i][index]);
            else col.push(null);
        } else {
            for(var i = this.data.length - Math.abs(start); i < this.data.length; i += 1)if (this.data[i]) col.push(this.data[i][index]);
            else col.push(null);
        }
        // Zero length
        if (typeof length === "number" && length === 0) col = [];
        else {
            // Positive length
            if (typeof length === "number" && length > 0) col = col.slice(0, length);
            else if (typeof length === "number" && length < 0) for(var i = 0; i < Math.abs(length); ++i)col.pop();
        }
        return col;
    };
    // Fetch the CSV file
...
//# sourceMappingURL=index.2e61b263.js.map
