...
RGraph.Horseshoe = function(conf) {
    ...
    //
    if (this.value <= 0.0000001) this.value = 0.0000001;
    ...
    this.set = function(name) {
        ...
        if (typeof name === "string") name = this.properties_lowercase_map[name.toLowerCase()] || name;
        // Set the colorsParsed flag to false if the colors
        // property is being set
        if (name === "colors" || name === "backgroundColor") this.colorsParsed = false;
    ...
    this.draw = function() {
        ...
        // Install clipping
        //
        if (!RGraph.isNull(this.properties.clip)) RGraph.clipTo.start(this, this.properties.clip);
        //
        ...
        // End clipping
        //
        if (!RGraph.isNull(this.properties.clip)) RGraph.clipTo.end();
        //
    ...
    this.drawMeter = function() {
        ...
        this.path("b a % % % 0 6.29 false a % % % 6.29 0 true f %", this.centerx, this.centery, this.radius, this.centerx, this.centery, this.radius - properties.width, properties.colors[1]);
        // Draw the black indicator line
        this.path("b a % % % % % false a % % % % % true f %", this.centerx, this.centery, this.radius, RGraph.TWOPI - RGraph.HALFPI, angle, this.centerx, this.centery, this.radius - properties.width, angle, RGraph.TWOPI - RGraph.HALFPI, properties.colors[0]);
        // Draw the bulb at the START of the indicator line
        this.path("b lw 3 a % % % 0 % false f % s white", this.centerx, this.centery - this.radius + properties.width / 2, typeof properties.endsRadius === "number" ? properties.endsRadius : properties.width * 1.5, RGraph.TWOPI, typeof properties.endsColor === "string" ? properties.endsColor : properties.colors[0]);
        // Draw the bulb at the END of the indicator line
        // (get the coordsinates to draw it at first).
        var coords = RGraph.getRadiusEndPoint(this.centerx, this.centery, angle, this.radius - properties.width / 2);
        this.path("b lw 3 a % % % 0 % false f % s white", coords[0], coords[1], typeof properties.endsRadius === "number" ? properties.endsRadius : properties.width * 1.5, RGraph.TWOPI, typeof properties.endsColor === "string" ? properties.endsColor : properties.colors[0]);
    ...
    this.drawLabels = function() {
        ...
        if (properties.labelsCenterSpecific) properties.labelsCenterSpecific = RGraph.labelSubstitution({
            object: this,
    ...
    this.getObjectByXY = function(e) {
        ...
        if (radius > this.radius) return null;
        return this;
    };
    //
    ...
    this.getAngle = function(value) {
        ...
        // An event object has been given
            ...
            var mouseX = value.offsetX, mouseY = value.offsetY;
        ...
        if (angle > RGraph.PI + RGraph.HALFPI) angle -= RGraph.TWOPI;
        return angle;
    };
    //
    ...
    this.grow = function() {
        ...
        var opt = arguments[0] || {}, frames = opt.frames || 30, frame = 0, diff = obj.value - obj.currentValue, step = diff / frames, callback = arguments[1] || function() {}, initial = obj.currentValue;
        function iterator() {
    ...
    this.clipToScaleWorker = function(clip) {
        ...
        var a1 = this.getAngle(from), a2 = this.getAngle(to);
        // Change the radius if the number is "min"
        if (RegExp.$1 === "min") a1 = this.getAngle(this.min);
        // Change the radius if the number is "max"
        if (RegExp.$2 === "max") a2 = this.getAngle(this.max);
        this.path("sa b    m % %    a % % % % % false    c cl", this.centerx, this.centery, this.centerx, this.centery, Math.max(this.canvas.width, this.canvas.height), a1, a2);
    };
    //
    // Register the object
    //
...
//# sourceMappingURL=index.e1dc460c.js.map
