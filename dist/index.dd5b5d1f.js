    ...
    RGraph.SVG.Bar = function(conf) {
        ...
        this.draw = function() {
            ...
            for(var i = 0; i < this.data_seq.length; ++i){
                if (typeof properties.errorbars[i] === "undefined" || RGraph.SVG.isNullish(properties.errorbars[i])) properties.errorbars[i] = {
                    max: null,
                    min: null
                };
            ...
            if (properties.adjustable) {
                ...
                RGraph.SVG.Bar.adjusting_mousemove_chart_update_function = function(opt) {
                    ...
                    if (RGraph.SVG.isNumber(index)) {
                        ...
                        // Grouped chart
                        if (RGraph.SVG.isNumber(data_subindex) && data_subindex >= 0) opt.object.data[data_index][data_subindex] = value;
                        else opt.object.data[data_index] = value;
                        opt.object.coords[index].element.setAttribute("data-value", value);
                    }
                };
                ...
                RGraph.SVG.runOnce("svg-bar-adjusting-window-mouseup-listener-" + this.id, function() {
                    window.addEventListener("mouseup", function(e) {
                        for(let i = 0, objects = RGraph.SVG.OR.objects; i < objects.length; ++i)if (objects[i].adjusting_mousedown) {
                            // Fire the beforedraw event
        ...
        this.drawBars = function() {
            ...
            for(var i = 0, sequentialIndex = 0; i < this.data.length; ++i, ++sequentialIndex){
                ...
                if (typeof this.data[i] === "number") {
                    ...
                    if (!RGraph.SVG.isNullish(properties.tooltips) && (!RGraph.SVG.isNullish(properties.tooltips[sequentialIndex]) || typeof properties.tooltips === "string")) {
                        var obj = this;
                        //
        ...
        this.getIndexByX = function(e) {
            ...
            for(let i = 0; i < this.coords.length; ++i){
                if (x >= this.coords[i].x && x <= this.coords[i].x + this.coords[i].width) return i;
            }
            return null;
        };
        //
                ...
                for(var i = 0, len = obj.coords.length; i < len; ++i){
                    ...
                    if (frame > opt.startFrames[i]) {
                        ...
                        if (properties.grouping === "stacked") {
                            ...
                            var indexes = RGraph.SVG.sequentialIndexToGrouped(seq, obj.data);
                            if (indexes[1] > 0) el.setAttribute("y", parseFloat(obj.coords[i - 1].element.getAttribute("y")) - height + 1);
                        }
                        if (properties.variant === "3d") {
                            // Add the 3D sides to the bar (again)
                            obj.drawSide3dFace({
        ...
        this.tooltipsFormattedCustom = function(specific, index) {
            if (typeof this.data[0] === "object") var label = !RGraph.SVG.isNullish(properties.tooltipsFormattedKeyLabels) && typeof properties.tooltipsFormattedKeyLabels === "object" && properties.tooltipsFormattedKeyLabels[index] ? properties.tooltipsFormattedKeyLabels[index] : "";
        ...
        this.clipToScaleWorker = function(clipPath) {
            ...
            var width = this.width, y1 = this.getYCoord(from), y2 = this.getYCoord(to), height = Math.abs(y2 - y1), x = 0, y = Math.min(y1, y2);
...
//# sourceMappingURL=index.dd5b5d1f.js.map
