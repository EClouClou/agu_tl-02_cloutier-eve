...
(function(win, doc, undefined) {
    RGraph.SVG.HBar = function(conf) {
        ...
        for(var i in this.properties)if (typeof i === "string") this.properties_lowercase_map[i.toLowerCase()] = i;
        //
        ...
        this.draw = function() {
            ...
            if (properties.marginLeftAuto) {
                ...
                if (properties.marginLeft < 15) properties.marginLeft = 15;
            }
            ...
            if (properties.yaxisPosition === "right" && properties.marginRightAuto !== false) {
                ...
                if (properties.marginRight < 15) properties.marginRight = 15;
            }
            ...
            for(var i = 0, max = 0; i < this.data.length; ++i){
                if (typeof this.data[i] === "number") values.push(this.data[i]);
                else if (RGraph.SVG.isArray(this.data[i]) && properties.grouping === "grouped") values.push(RGraph.SVG.arrayMax(this.data[i]));
                else if (RGraph.SVG.isArray(this.data[i]) && properties.grouping === "stacked") values.push(RGraph.SVG.arraySum(this.data[i]));
            }
            var max = RGraph.SVG.arrayMax(values);
            // A custom, user-specified maximum value
            if (typeof properties.xaxisScaleMax === "number") max = properties.xaxisScaleMax;
            ...
            if (this.mirrorScale) this.scale = RGraph.SVG.getScale({
                object: this,
                numlabels: properties.xaxisLabelsCount,
                unitsPre: properties.xaxisScaleUnitsPre,
                unitsPost: properties.xaxisScaleUnitsPost,
            ...
            if (this.properties.clip) {
                ...
                this.svgAllGroup.setAttribute("clip-path", "url(#{1})".format(this.clipid));
            } else {
            ...
            if (this.properties.line) this.drawLine();
            // Draw the key
            if (properties.key, RGraph.SVG.drawKey) RGraph.SVG.drawKey(this);
            else if (!RGraph.SVG.isNullish(properties.key)) alert("The drawKey() function does not exist - have you forgotten to include the key library?");
        ...
        this.create = function(str) {
            ...
            // been given
            if (arguments[1]) def.parent = arguments[1];
        ...
        this.yaxisLabelSubstitution = function() {
            if (properties.yaxisLabels && properties.yaxisLabels.length) {
                ...
                // into an array.
                //
                if (typeof properties.yaxisLabels === "string") properties.yaxisLabels = RGraph.SVG.arrayPad({
                    array: [],
                ...
                // Label substitution
                //
                for(var i = 0; i < properties.yaxisLabels.length; ++i)properties.yaxisLabels[i] = RGraph.SVG.labelSubstitution({
                    object: this,
                    text: properties.yaxisLabels[i],
                    index: i,
                    value: this.data[i],
        ...
        this.drawBars = function() {
            ...
            for(var i = 0, sequentialIndex = 0; i < this.data.length; ++i, ++sequentialIndex){
                ...
                if (typeof this.data[i] === "number") {
                    var outerSegment = (this.graphHeight - properties.marginInnerTop - properties.marginInnerBottom) / this.data.length, width1 = this.getWidth(this.data[i]), height = (this.graphHeight - properties.marginInnerTop - properties.marginInnerBottom) / this.data.length - properties.marginInner - properties.marginInner, x = this.getXCoord(this.scale.min < 0 && this.scale.max < 0 || this.scale.min > 0 && this.scale.max > 0 ? this.scale.min : 0) - (this.data[i] < 0 ? width1 : 0), y = properties.marginTop + properties.marginInnerTop + properties.marginInner + outerSegment * i;
                    // Allow for the Y axis to be positioned on the right hand side
                    if (properties.yaxisPosition === "right" && this.scale.min >= 0) x = this.getXCoord(this.data[i]);
                    ...
                    // side
                    if (this.scale.min < 0 && this.scale.max < 0) x = this.width - properties.marginRight - width1;
                    // Adjust for a negative value
                    if (this.mirrorScale && properties.yaxisPosition === "right") {
                        if (this.data[i] > 0) x = this.getXCoord(0) - width1;
                        else x = this.getXCoord(0);
                    }
                    // If the X axis is right, move the bar left
                    if (this.data[i] > 0 && properties.yaxisPosition === "right") x = this.getXCoord(0) - width1;
                ...
                //
                    ...
                    var outerSegment = (this.graphHeight - properties.marginInnerTop - properties.marginInnerBottom) / this.data.length, innerSegment = outerSegment - 2 * properties.marginInner;
                    // Loop through the group
                    for(var j = 0; j < this.data[i].length; ++j, ++sequentialIndex){
                        var width1 = Math.abs(this.data[i][j] / (this.max - this.min) * this.graphWidth), height = (innerSegment - (this.data[i].length - 1) * properties.marginInnerGrouped) / this.data[i].length, y = properties.marginTop + properties.marginInner + properties.marginInnerTop + outerSegment * i + j * height + j * properties.marginInnerGrouped, x = this.getXCoord(0) - (this.data[i][j] < 0 ? width1 : 0);
                        ...
                        if (properties.yaxisPosition === "right" && this.scale.min < 0 && this.scale.max >= 0) {
                            if (this.data[i][j] < 0) x = this.getXCoord(0);
                            else x = this.getXCoord(this.data[i][j]);
                        }
                        // Fixes an odd bug
                        //if (this.mirrorScale && properties.yaxisPosition === 'right') {
                        //    if (this.data[i][j] > 0) {
                        ...
                        if (properties.colorsSequential) {
                            if (properties.colors[sequentialIndex]) fill = properties.colors[sequentialIndex];
                        } else if (properties.colors[j]) fill = properties.colors[j];
                        else fill = properties.colors[properties.colors.length - 1];
                        var rect = RGraph.SVG.create({
                            svg: this.svg,
                            type: "rect",
                            parent: this.svgAllGroup,
                        ...
                        if (!RGraph.SVG.isNullish(properties.tooltips) && (properties.tooltips[sequentialIndex] || typeof properties.tooltips === "string")) {
                            ...
                            //
                            (function(idx, seq) {
                                var indexes1 = RGraph.SVG.sequentialIndexToGrouped(seq, obj.data);
                                rect.addEventListener(properties.tooltipsEvent.replace(/^on/, ""), function(e) {
                                    obj.removeHighlight();
                                    // Show the tooltip
                                    RGraph.SVG.tooltip({
                ...
                //
                    ...
                    for(var j = 0; j < this.data[i].length; ++j, ++sequentialIndex){
                        var outerHeight = (this.graphHeight - properties.marginInnerTop - properties.marginInnerBottom) / this.data.length, width1 = Math.abs(this.data[i][j] / (this.max - this.min) * this.graphWidth), height = outerHeight - 2 * properties.marginInner, y = properties.marginTop + properties.marginInner + properties.marginInnerTop + outerHeight * i;
                        if (properties.yaxisPosition === "right") x -= width1;
        ...
        this.getXCoord = function(value) {
            ...
            x *= this.graphWidth;
            if (properties.yaxisPosition === "right") x = this.width - properties.marginRight - x;
            else x += properties.marginLeft;
            return x;
        };
        //
        // This function can be used to retrieve the relevant X coordinate for a
        ...
        this.highlight = function(rect) {
            var x = parseFloat(rect.getAttribute("x")) - 0.5, y = parseFloat(rect.getAttribute("y")) - 0.5, width1 = parseFloat(rect.getAttribute("width")) + 1, height = parseFloat(rect.getAttribute("height")) + 1;
        ...
        this.parseColors = function() {
            ...
            properties.lineColor = RGraph.SVG.parseColorLinear({
                object: this,
                color: properties.lineColor
            });
            properties.lineFilledColor = RGraph.SVG.parseColorLinear({
                object: this,
                color: properties.lineFilledColor
            });
        };
        //
        ...
        this.drawLabelsAbove = function() {
            ...
            if (properties.labelsAbove) {
                ...
                for(var i = 0; i < this.coords.length; ++i){
                    ...
                    var indexes1 = RGraph.SVG.sequentialIndexToGrouped(i, this.data);
                    if (RGraph.SVG.isArray(this.data[indexes1[0]]) && properties.grouping === "stacked") {
                        if (indexes1[1] + 1 === this.data[indexes1[0]].length) {
                            value = RGraph.SVG.arraySum(this.data[indexes1[0]]);
                    ...
                    var halign = properties.labelsAboveHalign, valign = properties.labelsAboveValign;
                    ...
                    var x = value >= 0 ? parseFloat(this.coords[i].element.getAttribute("x")) + 7 + properties.labelsAboveOffsetx : parseFloat(this.coords[i].element.getAttribute("x") - 7 - properties.labelsAboveOffsetx), y = parseFloat(this.coords[i].element.getAttribute("y")) + parseFloat(this.coords[i].element.getAttribute("height") / 2) + properties.labelsAboveOffsety, width1 = dimensions[0], height = dimensions[1], halign = value >= 0 ? "left" : "right";
                    // Corner case
                    if (properties.yaxisPosition === "left" && properties.grouping === "grouped") x = parseFloat(this.coords[i].element.getAttribute("x")) + parseFloat(this.coords[i].element.getAttribute("width")) + 7 + properties.labelsAboveOffsetx;
                    // ADjust the values if the Y axis is on the RHS
                    if (properties.yaxisPosition === "right") x = value >= 0 ? parseFloat(this.coords[i].element.getAttribute("x")) - 7 - properties.labelsAboveOffsetx : parseFloat(this.coords[i].element.getAttribute("x") + 7 + properties.labelsAboveOffsetx), halign = value >= 0 ? "right" : "left";
                    else if (RGraph.SVG.isArray(this.data[indexes1[0]]) && properties.grouping === "stacked" && properties.yaxisPosition === "left") x += this.coords2[indexes1[0]][indexes1[1]].width;
                    // Another corner case
                    if (properties.yaxisPosition === "right" && properties.grouping === "grouped" && properties.xaxisScaleMax > 0 && properties.xaxisScaleMin < 0) {
                        var value = this.coords[i].element.getAttribute("data-value");
                        if (value < 0) x = this.getXCoord(value) + 7;
                        else x = this.getXCoord(value) - 7;
                    }
                    // Another corner case
                    if (properties.yaxisPosition === "left" && properties.grouping === "grouped" && properties.xaxisScaleMax > 0 && properties.xaxisScaleMin < 0) {
                        var value = this.coords[i].element.getAttribute("data-value");
                        if (value < 0) x = this.getXCoord(value) - 7;
                        else x = this.getXCoord(value) + 7;
                    }
                    ...
                    if (properties.grouping === "stacked" && typeof this.data[indexes1[0]] === "number" && properties.yaxisPosition === "left") x += parseInt(this.coords[i].element.getAttribute("width"));
        ...
        this.drawLabelsInbar = function() {
            ...
            if (properties.labelsInbar) {
                // Default alignment
                var valign = properties.labelsInbarValign, halign = properties.labelsInbarHalign;
                // Get the text configuration for the labels
                var textConf = RGraph.SVG.getTextConf({
                    object: this,
                    prefix: "labelsInbar"
                ...
                var data = RGraph.SVG.arrayLinearize(this.data);
                for(var i = 0; i < this.coords.length; ++i)if (RGraph.SVG.isNumber(data[i]) && !isNaN(data[i])) {
                    ...
                    var x = parseFloat(el.getAttribute("x")) + parseFloat(el.getAttribute("width")) / 2 + properties.labelsInbarOffsetx, y = parseFloat(el.getAttribute("y")) + parseFloat(el.getAttribute("height")) / 2 + properties.labelsInbarOffsety, width1 = dimensions[0], height = dimensions[1];
                    //
                    // Horizontal alignment
                    //
                    ...
                    if (RGraph.SVG.isArray(properties.labelsInbarSpecific) && (RGraph.SVG.isString(properties.labelsInbarSpecific[i]) || RGraph.SVG.isNumber(properties.labelsInbarSpecific[i]))) str = properties.labelsInbarSpecific[i];
                    var text = RGraph.SVG.text({
                        object: this,
                        parent: this.svgAllGroup,
                        tag: "labels.inbar",
            ...
            var iterate = function() {
                for(var i = 0, seq = 0, len = obj.coords.length; i < len; ++i, ++seq){
                    ...
                    if (typeof data[i] === "number") {
                        ...
                        obj.coords[seq].element.setAttribute("width", width);
                        // Set the correct Y coord on the object
                        obj.coords[seq].element.setAttribute("x", data[i] > 0 ? obj.getXCoord(0) - (properties.yaxisPosition === "right" ? width : 0) : properties.xaxisScaleMin < 0 && properties.xaxisScaleMax > 0 ? properties.yaxisPosition === "right" ? obj.getXCoord(0) : obj.getXCoord(0) - width : obj.getXCoord(0));
                        ...
                        for(var j = 0, len2 = data[i].length; j < len2; ++j, ++seq){
                            ...
                            obj.coords[seq].element.setAttribute("width", width);
                            if (properties.yaxisPosition === "right") {
                                if (properties.grouping === "stacked") obj.coords[seq].element.setAttribute("x", obj.getXCoord(0) - accumulativeWidth);
                                else obj.coords[seq].element.setAttribute("x", obj.getXCoord(0) - (obj.coords[seq].element.getAttribute("data-value") < 0 ? 0 : width));
                            } else obj.coords[seq].element.setAttribute("x", properties.grouping === "stacked" ? obj.getXCoord(0) + (accumulativeWidth - width) : properties.grouping === "grouped" && obj.coords[seq].element.getAttribute("data-value") < 0 ? obj.getXCoord(0) - width : obj.getXCoord(0));
                        }
                        //
                        // Set the height and Y cooord of the backfaces if necessary
                        //
                        if (obj.stackedBackfaces[i]) {
                            obj.stackedBackfaces[i].setAttribute("width", accumulativeWidth);
                            obj.stackedBackfaces[i].setAttribute("x", properties.yaxisPosition === "right" ? obj.getXCoord(0) - accumulativeWidth : obj.getXCoord(0));
                        }
        ...
        this.wave = function() {
            ...
            opt.startFrames = [];
            opt.counters = [];
            var framesperbar = opt.frames / 3, frame = -1, callback = opt.callback || function() {}, width1;
            for(var i = 0, len = this.coords.length; i < len; i += 1){
                ...
                if (properties.grouping === "stacked" && obj.stackedBackfaces[i]) obj.stackedBackfaces[i].setAttribute("width", 0);
            }
            // Edge-case
            ...
            function iterator() {
                ++frame;
                var group = 0;
                for(var i = 0, len = obj.coords.length; i < len; i += 1)if (frame > opt.startFrames[i]) {
                    var originalWidth = obj.coords[i].element.getAttribute("data-original-width"), value = parseFloat(obj.coords[i].element.getAttribute("data-value")), seq = i;
                    indexes = RGraph.SVG.sequentialIndexToGrouped(i, obj.data);
                    if (indexes[0] !== group) group = indexes[0];
                    obj.coords[i].element.setAttribute("width", width1 = Math.min((frame - opt.startFrames[i]) / framesperbar * originalWidth, originalWidth));
                    stackedAccumulativeWidth += width1;
                    if (properties.yaxisPosition === "right") {
                        if (properties.grouping === "stacked") {
                            if (indexes[1] === 0) {
                                obj.coords[i].element.setAttribute("x", obj.width - properties.marginRight - width1);
                                var previousX1 = obj.coords[i].element.getAttribute("x");
                            } else obj.coords[i].element.setAttribute("x", previousX1 - width1);
                        } else obj.coords[i].element.setAttribute("x", value >= 0 ? obj.getXCoord(0) - width1 : obj.getXCoord(0));
                    } else obj.coords[i].element.setAttribute("x", value >= 0 ? obj.getXCoord(0) : obj.getXCoord(0) - width1);
                    if (properties.grouping === "stacked" && RGraph.SVG.isArray(obj.data[indexes[0]])) {
                        ...
                        ////////////////////////////////////
                        if (properties.yaxisPosition === "left" && indexes[1] > 0) obj.coords[i].element.setAttribute("x", parseInt(obj.coords[i - 1].element.getAttribute("x")) + parseInt(obj.coords[i - 1].element.getAttribute("width")));
                        ...
                        previousX1 = obj.coords[i].element.getAttribute("x");
        ...
        this.tooltipSubstitutions = function(opt) {
            var indexes1 = RGraph.SVG.sequentialIndexToGrouped(opt.index, this.data);
            return {
                index: indexes1[1],
                dataset: indexes1[0],
                sequentialIndex: opt.index,
                value: typeof this.data[indexes1[0]] === "number" ? this.data[indexes1[0]] : this.data[indexes1[0]][indexes1[1]],
                values: typeof this.data[indexes1[0]] === "number" ? [
                    this.data[indexes1[0]]
                ] : this.data[indexes1[0]]
            };
        };
        //
        ...
        //
        this.tooltipsFormattedCustom = function(specific, index1) {
            if (typeof this.data[0] === "object") var label = !RGraph.SVG.isNullish(properties.tooltipsFormattedKeyLabels) && typeof properties.tooltipsFormattedKeyLabels === "object" && properties.tooltipsFormattedKeyLabels[index1] ? properties.tooltipsFormattedKeyLabels[index1] : "";
            else var label = !RGraph.SVG.isNullish(properties.tooltipsFormattedKeyLabels) && typeof properties.tooltipsFormattedKeyLabels === "object" && properties.tooltipsFormattedKeyLabels[specific.index] ? properties.tooltipsFormattedKeyLabels[specific.index] : "";
            return {
                label: label
            };
        };
        ...
        this.positionTooltipStatic = function(args) {
            var obj = args.object, e = args.event, tooltip = args.tooltip, index1 = args.index, svgXY = RGraph.SVG.getSVGXY(obj.svg), coords = this.coords[args.index];
        ...
        this.drawLine = function() {
            if (this.properties.lineShadow) RGraph.SVG.setShadow({
                object: this,
            ...
            if (this.properties.lineSpline) {
                ...
                if (this.properties.lineFilled) {
                    ...
                    if (this.properties.yaxisPosition === "right") {
                        fillAreaCoordinates.push([
                            this.width - this.properties.marginRight,
                ...
                if (RGraph.SVG.isNumber(this.data[0])) {
                    if (this.properties.yaxisPosition === "right") d = "M {1} {2} ".format(this.coords[0].x, this.coords[0].y + this.coords[0].height / 2);
                    else {
                        var v = Number(this.coords[0].element.getAttribute("data-value"));
                        d = "M {1} {2} ".format(this.coords[0].x + (v < 0 ? 0 : this.coords[0].width), this.coords[0].y + this.coords[0].height / 2);
                    }
                }
                ...
                for(let i = 0; i < this.coords.length; ++i){
                    ...
                    d += "{1} {2} {3} ".format(action, x, y);
                }
                if (properties.lineFilled) RGraph.SVG.create({
                    svg: this.svg,
                    type: "path",
                    parent: this.svgAllGroup,
                    attr: {
                        d: d + " L " + (properties.yaxisPosition === "right" ? [
                            this.width - this.properties.marginRight,
                            y
                        ].join(" ") : [
                            this.properties.marginLeft,
                            y
                        ].join(" ")) + " L " + (properties.yaxisPosition === "right" ? [
                            this.width - this.properties.marginRight,
            ...
            this.coords.forEach(function(v, k, arr) {
                if (typeof obj.properties.lineTickmarksStyle === "string") {
                    ...
                    switch(obj.properties.lineTickmarksStyle){
                        ...
                        case "endcircle":
                            if (obj.properties.lineTickmarksStyle.indexOf("end") >= 0 && isEndTick || obj.properties.lineTickmarksStyle.indexOf("end") === -1) {
                                var x = v.x;
                                var y = v.y + v.height / 2;
                                // Adjust the tickmark if the
                                // Y-axis position is on the left
                                // and the value is positive
                                if (obj.properties.yaxisPosition === "left" && Number(v.element.getAttribute("data-value")) > 0) x += v.width;
                                // Adjust the tickmark if the
                                // Y-axis position is on the right
                                // and the value is negative
                                if (obj.properties.yaxisPosition === "right" && Number(v.element.getAttribute("data-value")) < 0) x += v.width;
                                RGraph.SVG.create({
                                    svg: obj.svg,
                                    type: "circle",
                                    parent: obj.svgAllGroup,
                                ...
                                if (obj.properties.lineTickmarksStyle.indexOf("filled") < 0) RGraph.SVG.create({
                                    svg: obj.svg,
                                    type: "circle",
                                    parent: obj.svgAllGroup,
                                    attr: {
                                        cx: x,
                                        cy: y,
                                        r: RGraph.SVG.isNumber(obj.properties.lineTickmarksLinewidth) ? obj.properties.lineTickmarksSize - obj.properties.lineTickmarksLinewidth : obj.properties.lineTickmarksSize - 3,
                                        fill: "white",
                                        "clip-path": obj.isTrace ? "url(#trace-effect-clip)" : ""
                                    }
                                });
                            ...
                            if (obj.properties.lineTickmarksStyle.indexOf("end") >= 0 && isEndTick || obj.properties.lineTickmarksStyle.indexOf("end") === -1) {
                                ...
                                if (obj.properties.lineTickmarksStyle.indexOf("filled") < 0) RGraph.SVG.create({
                                    ...
                                    attr: {
                                        ...
                                        width: RGraph.SVG.isNumber(obj.properties.lineTickmarksLinewidth) ? obj.properties.lineTickmarksSize * 2 - 2 * obj.properties.lineTickmarksLinewidth : obj.properties.lineTickmarksSize * 2 - 3 - 3,
                                        height: RGraph.SVG.isNumber(obj.properties.lineTickmarksLinewidth) ? obj.properties.lineTickmarksSize * 2 - 2 * obj.properties.lineTickmarksLinewidth : obj.properties.lineTickmarksSize * 2 - 3 - 3,
                                        fill: "white",
                                        "clip-path": obj.isTrace ? "url(#trace-effect-clip)" : ""
                                    }
                                });
            ...
            function Spline(coords, opt = {}) {
                ...
                var yCoords = [], interval = (obj.height - obj.properties.marginTop - obj.properties.marginBottom) / coords.length;
                path = [];
                //
                ...
                // format
                //
                for(var i = 0; i < coords.length; ++i)coords[i] = Number(coords[i].x) + (obj.properties.yaxisPosition === "right" ? obj.data[i] < 0 ? Number(coords[i].width) : 0 : obj.data[i] < 0 ? 0 : Number(coords[i].width));
                //
                ...
                for(var j = 1; j < P.length - 2; ++j)for(var t = 0; t < 10; ++t){
                    var xCoord = Spline(t / 10, P[j - 1], P[j], P[j + 1], P[j + 2]);
                    yCoords.push((j - 1) * interval + t * (interval / 10) + obj.properties.marginTop + halfsection);
                    path.push([
                        xCoord,
                        yCoords[yCoords.length - 1]
                    ]);
                    if (typeof index == "number") coordsSpline[0].push([
                ...
                if (obj.properties.yaxisPosition === "left" && obj.coords[j - 1].element.getAttribute("data-value") > 0) last[0] += obj.coords[obj.coords.length - 1].width;
                // Adjust the position of the last point
                if (obj.properties.yaxisPosition === "right" && obj.coords[j - 1].element.getAttribute("data-value") > 0) last[0] -= obj.coords[obj.coords.length - 1].width;
                path.push([
                    last[0],
                    last[1]
                ]);
                ...
                //
                if (opt.return) // The 'path' variable appears to contain all
                // of the coordinates.
                return path;
                else RGraph.SVG.create({
                    svg: obj.svg,
                    parent: obj.svgAllGroup,
                    type: "path",
                    attr: {
        ...
        //
        this.trace = function() {
            var opt = arguments[0] || {}, frame = 1, frames = opt.frames || 60, obj = this;
            this.isTrace = true;
            this.draw();
            ...
            var iterator = function() {
                ...
                if (frame <= frames) RGraph.SVG.FX.update(iterator);
                else {
                    // Remove the clippath
        ...
        this.clipToScaleWorker = function(clipPath) {
            ...
            // library
            if (RegExp.$1 === "min") from = this.min;
            else from = Number(RegExp.$1);
            if (RegExp.$2 === "max") to = this.max;
            else to = Number(RegExp.$2);
            var height = this.height, x1 = this.getXCoord(from), x2 = this.getXCoord(to), width1 = Math.abs(x2 - x1), y = 0, x = Math.min(x1, x2);
            // Increase the width if the maximum value is "max"
            if (RegExp.$2 === "max") width1 += this.properties.marginRight;
            // Increase the height if the minimum value is "min"
            if (RegExp.$1 === "min") {
                x = 0;
                width1 += this.properties.marginLeft;
            }
            RGraph.SVG.create({
                svg: this.svg,
                type: "rect",
            ...
            this.svgAllGroup.setAttribute("clip-path", "url(#" + clipPath.id + ")");
        };
        //
        // This function allows the drawing of custom lines
        //
        this.drawVerticalLines = function() {
            var lines = this.properties.verticalLines, avg, x, y, label, halign, valign, hmargin = 5, vmargin = 10, position, textFont, textSize, textColor, textBold, textItalic, data, linewidth, data_linear;
            if (lines) {
                ...
                for(var i = 0; i < this.properties.verticalLines.length; ++i){
                    var conf = lines[i], textFont = conf.labelFont || this.properties.textFont, textColor = conf.labelColor || defaults.labelColor, textSize = conf.labelSize || this.properties.textSize, textBold = conf.labelBold ? conf.labelBold : this.properties.textBold, textItalic = conf.labelItalic ? conf.labelItalic : this.properties.textItalic;
                    if (typeof conf.value === "number") x = this.getXCoord(conf.value);
                    else {
                        conf.value = "average";
                    ...
                    if (conf.dashed === true || RGraph.SVG.isUndefined(conf.dashed) && RGraph.SVG.isUndefined(conf.dotted)) linedash = "6 6";
                    //
                    // Draw the line
                    //
                    RGraph.SVG.create({
                    ...
                    } else var num = Number(conf.value).toFixed(conf.labelValueDecimals);
                    ...
                    if (!conf.labelPosition) conf.labelPosition = "top";
                    // Determine the position of the label
                    if (conf.labelPosition === "bottom") var y = this.height - this.properties.marginBottom + (conf.labelOffsety || 0) + 7;
...
//# sourceMappingURL=index.f60f43ff.js.map
