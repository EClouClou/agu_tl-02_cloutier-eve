    ...
    RGraph.SVG.Pie = function(conf) {
        ...
        this.draw = function() {
            ...
            if (typeof properties.exploded === "number" && properties.exploded > 0) {
                ...
                properties.exploded = [];
                for(var i = 0; i < this.data.length; ++i)properties.exploded[i] = val;
            }
            ...
            if (properties.labels && properties.labels.length) {
                if (typeof properties.labels === "string") properties.labels = RGraph.SVG.arrayPad({
                    array: [],
                ...
                // Label substitution
                //
                for(var i = 0; i < properties.labels.length; ++i)properties.labels[i] = RGraph.SVG.labelSubstitution({
                    object: this,
            ...
            if (properties.labelsSticks) this.drawLabelsSticks();
        ...
        this.drawSegments = function(opt) {
            var start = 0, end = 0, angle = 0, sum = RGraph.SVG.arraySum(this.data), segment = 0;
            ...
            for(var i = 0, len = this.data.length; i < len; ++i){
                ...
                var explosionX = explosion[1], explosionY = explosion[0];
                this.angles[i] = {
                    start: start,
                    end: end,
                    angle: end - start,
            ...
            if (opt.shadow) RGraph.SVG.setShadow({
                object: this,
            ...
            for(var i = 0; i < this.angles.length; ++i){
                ...
                if (properties.donut) {
                    ...
                    path = path + " L {1} {2} ".format(xy[0] + this.angles[i].cx, xy[1] + this.angles[i].cy) + donut_path + " Z";
                } else path = path + " L {1} {2} ".format(this.angles[i].cx, this.angles[i].cy) + " Z";
                ...
                if (properties.shadow && opt.shadow) this.shadowNodes[i] = arc;
                else this.nodes[i] = arc;
                if (properties.tooltips && (properties.tooltips[i] || typeof properties.tooltips === "string") && (!opt.shadow || !properties.shadow)) {
                    ...
                    if (properties.tooltipsEvent !== "mousemove") properties.tooltipsEvent = "click";
                    (function(index, obj) {
                        arc.addEventListener(properties.tooltipsEvent, function(e) {
                            ...
                            if (tooltip && properties.tooltipsEvent === "mousemove" && index === tooltip.__index__) return;
                            obj.removeHighlight();
                            // Show the tooltip
                            RGraph.SVG.tooltip({
                                object: obj,
                            ...
                            if (properties.tooltipsEvent === "mousemove") highlight.style.cursor = "pointer";
                        }, false);
                        // Install the event listener that changes the
                        // cursor if necessary
                        if (properties.tooltipsEvent === "click") arc.addEventListener("mousemove", function(e) {
            ...
            // top of any shadow
            //
            if (properties.shadow && opt.shadow) this.redrawSegments();
        };
        //
        ...
        this.drawLabels = function() {
            var angles = this.angles, labels = properties.labels, textConf = RGraph.SVG.getTextConf({
                object: this,
                prefix: "labels"
            });
            for(var i = 0; i < angles.length; ++i){
                ...
                var x = endpoint[0] + angles[i].cx, y = endpoint[1] + angles[i].cy, valign, halign;
        ...
        this.drawIngraphLabels = function() {
            if (properties.labelsIngraph) {
                ...
                for(var i = 0; i < this.angles.length; ++i){
                    // Some defaults
                    var halign = properties.labelsIngraphHalign || "center", valign = properties.labelsIngraphValign || "center", bgcolor = properties.labelsIngraphBackground || "transparent", decimals = properties.labelsIngraphDecimals || 0, padding = typeof properties.labelsIngraphBackground === "string" ? 3 : 0;
                    ...
                    if (typeof properties.labelsIngraphSpecific === "object" && properties.labelsIngraphSpecific) {
                        if (typeof properties.labelsIngraphSpecific[i] === "string") var str = properties.labelsIngraphSpecific[i];
                        else var str = "";
                    } else if (typeof properties.labelsIngraphFormatter === "function") var str = properties.labelsIngraphFormatter({
                        object: this,
                        number: this.data[i].toFixed(decimals)
                    });
                    else var str = RGraph.SVG.numberFormat({
        ...
        this.drawLabelsSticks = function() {
            var labels_right = [], labels_left = [], labels_coords = [];
            for(var i = 0; i < this.angles.length; ++i){
                var angle = this.angles[i].start + (this.angles[i].end - this.angles[i].start) / 2 - RGraph.SVG.TRIG.HALFPI, endpoint_inner = RGraph.SVG.TRIG.getRadiusEndPoint({
                    angle: angle,
                    r: this.radius + 5
                }), endpoint_outer = RGraph.SVG.TRIG.getRadiusEndPoint({
                    angle: angle,
                    r: this.radius + 30
                }), explosion = [
                ...
                var x, y;
                if (labels[index].text) var stick = RGraph.SVG.create({
                    svg: this.svg,
                    parent: this.svgAllGroup,
                    type: "path",
                    attr: {
                        d: "M {1} {2} L {3} {4}".format(this.centerx + endpoint_inner[0], this.centery + endpoint_inner[1], this.centerx + endpoint_outer[0], this.centery + endpoint_outer[1]),
                        stroke: "#999",
                        fill: "rgba(0,0,0,0)"
                    }
                });
            ...
            for(var i = 0; i < labels_right.length; ++i)if (labels_right[i] && labels_right[i].text) {
                ...
                labels_right[i].stick.setAttribute("d", str.format(x - 5, y));
            }
            // Loop through the LHS labels
            for(var i = 0; i < labels_left.length; ++i)if (labels_left[i] && labels_left[i].text) {
                x = this.centerx - this.radius - properties.labelsSticksOffset;
                ...
                labels_left[i].stick.setAttribute("d", str.format(x - 5, y));
            }
        };
        //
        ...
        this.highlight = function(segment) {
            ...
            } else var highlight = RGraph.SVG.create({
                svg: this.svg,
                parent: this.svgAllGroup,
                type: "path",
                attr: {
        ...
        this.parseColors = function() {
            ...
            if (colors) for(var i = 0; i < colors.length; ++i)colors[i] = RGraph.SVG.parseColorRadial({
                object: this,
                color: colors[i]
            });
            // Highlight fill
        ...
        this.roundrobin = this.roundRobin = function() {
            var obj = this, opt = arguments[0] || {}, data = RGraph.SVG.arrayClone(this.data, true), frame = 1, frames = opt.frames || 30, callback = typeof opt.callback === "function" ? opt.callback : function() {}, dataSum = RGraph.SVG.arraySum(this.data), textColor = properties.textColor, ingraph = properties.labelsIngraph, multiplier = 0;
            ...
            function iterator() {
                ...
                for(var i = 0; i < angles.length; ++i){
                    ...
                    var segment = (obj.angles[i].end - obj.angles[i].start) / 2, explodedX = Math.cos(obj.angles[i].start + segment - RGraph.SVG.TRIG.HALFPI) * (properties.exploded[i] || 0), explodedY = Math.sin(obj.angles[i].start + segment - RGraph.SVG.TRIG.HALFPI) * (properties.exploded[i] || 0);
                    var path = RGraph.SVG.TRIG.getArcPath({
                    ...
                    if (properties.donut) {
                        ...
                        path = path + " L {1} {2} ".format(xy[0] + obj.angles[i].cx, xy[1] + obj.angles[i].cy) + donut_path + " Z";
                    } else path = path + " L {1} {2} ".format(obj.angles[i].cx, obj.angles[i].cy) + " Z";
                    path = path + " L {1} {2} Z".format(obj.centerx + explodedX, obj.centery + explodedY);
                    if (obj.shadowNodes && obj.shadowNodes[i]) obj.shadowNodes[i].setAttribute("d", path);
        ...
        this.positionTooltipStatic = function(args) {
            var obj = args.object, e = args.event, tooltip = args.tooltip, index = args.index, svgXY = RGraph.SVG.getSVGXY(obj.svg), segment = this.angles[args.index], angle = segment.halfway, multiplier = 0.5;
            //
            // Determine the correct radius to use when calculating the
            // coordinates of the tooltip
            //
            if (properties.donut) // Determine the radius
            var radius = this.radius - properties.donutWidth + properties.donutWidth / 2;
            else var radius = this.radius * multiplier;
...
//# sourceMappingURL=index.9ffd6e17.js.map
