...
(function(win, doc, undefined) {
    ...
    RGraph.SVG.createSVG = function(opt) {
        ...
        if (container.__svg__) {
            ...
            // layers"
            for(var i1 = 1; i1 <= 10; ++i1)// Store a reference to the group on the chart object
            obj.layers["background" + i1] = container.__svg__["background" + i1];
        ...
        for(var i1 = 1; i1 <= 10; ++i1){
            ...
            obj.layers["background" + i1] = group;
            svg["background" + i1] = group;
        }
        return svg;
    };
    //
    ...
    RGraph.SVG.create = function(opt) {
        ...
        for(var o in opt.attr)if (typeof o === "string") {
            ...
            if ((opt.type === "a" || opt.type === "image") && o === "xlink:href") tag.setAttributeNS("http://www.w3.org/1999/xlink", o, String(opt.attr[o]));
                ...
                if (RGraph.SVG.isNullish(opt.attr[o])) opt.attr[o] = "";
    ...
    RGraph.SVG.create.parseStr = function(obj, str) {
        ...
        // double pipe
        str = str.replace(/stroke-dasharray="([0-9 ]+)"/, function(str, match1) {
            str = match1.replace(/ /, "||");
        ...
        for(var i1 = 1, attr = {}, name, value; i1 < parts.length; ++i1){
            [name, value] = parts[i1].split('="');
            if (name) {
    ...
    RGraph.SVG.create.pathString = function(coords, initial = "M") {
        var path = "{1} {2} {3}".format(initial, coords[0][0], coords[0][1]);
        for(var i1 = 0; i1 < coords.length; ++i1)path += ` L ${coords[i1][0]} ${coords[i1][1]}`;
        return path;
    };
    //
    ...
    RGraph.SVG.getMouseXY = function(e) {
        ...
        if (!e.target) return;
        var el = e.target, offsetX = 0, offsetY = 0, x1, y1;
    ...
    RGraph.SVG.drawXAxis = function(obj) {
        ...
        // Draw the axis
        if (properties.xaxis) {
            var y1 = obj.type === "hbar" ? obj.height - properties.marginBottom : obj.getYCoord(obj.scale.min < 0 && obj.scale.max < 0 ? obj.scale.max : obj.scale.min > 0 && obj.scale.max > 0 ? obj.scale.min : 0);
            var axis = RGraph.SVG.create({
                svg: obj.svg,
                parent: obj.svgAllGroup,
                type: "path",
                attr: {
                    d: "M{1} {2} L{3} {4}".format(properties.marginLeft, y1, obj.width - properties.marginRight, y1),
            ...
            // HBar X axis
            if (obj.type === "hbar") var width = obj.graphWidth / obj.data.length, x1 = properties.marginLeft, startY = obj.height - properties.marginBottom, endY = obj.height - properties.marginBottom + properties.xaxisTickmarksLength;
            else {
                var width = obj.graphWidth / obj.data.length, x1 = properties.marginLeft, startY = obj.getYCoord(0) - (properties.yaxisScaleMin < 0 ? properties.xaxisTickmarksLength : 0), endY = obj.getYCoord(0) + properties.xaxisTickmarksLength;
            ...
            if (properties.xaxisTickmarks) {
                ...
                if (properties.xaxisScale) {
                    ...
                    for(var i1 = 0; i1 < (typeof properties.xaxisLabelsPositionEdgeTickmarksCount === "number" ? properties.xaxisLabelsPositionEdgeTickmarksCount : obj.scale.numlabels + (properties.yaxis && properties.xaxisScaleMin === 0 ? 0 : 1)); ++i1){
                        if (obj.type === "hbar") var dataPoints = obj.data.length;
                        x1 = properties.marginLeft + (i1 + (properties.yaxis && properties.xaxisScaleMin === 0 && properties.yaxisPosition === "left" ? 1 : 0)) * (obj.graphWidth / obj.scale.numlabels);
                        ...
                        if (typeof properties.xaxisLabelsPositionEdgeTickmarksCount === "number") {
                            ...
                            x1 = gap * i1 + properties.marginLeft + gap;
                            // Allow for the Y axis being on the right so the tickmarks
                            // have to be adjusted
                            if (properties.yaxisPosition === "right") x1 -= gap;
                        }
                        // Don't draw a tick at the zero position
                        if (properties.yaxis && x1 < zeroXCoord + 2 && x1 > zeroXCoord - 2) continue;
                        // Don't draw a tick at the zero position (another case)
                        if (properties.yaxis && obj.min > 0 && obj.max > obj.min && i1 === 0) continue;
                        // Don't draw a tick at the zero position (another case)
                        if (properties.yaxis && obj.max < 0 && obj.min < obj.max && i1 === 5) continue;
                        RGraph.SVG.create({
                            svg: obj.svg,
                            parent: obj.svgAllGroup,
                            type: "path",
                            attr: {
                                d: "M{1} {2} L{3} {4}".format(x1, startY, x1, endY),
                                stroke: properties.xaxisColor,
                                "stroke-width": typeof properties.xaxisLinewidth === "number" ? properties.xaxisLinewidth : 1,
                                "shape-rendering": "crispEdges"
                            }
                    ...
                    if (properties.xaxisLabelsPosition === "section") {
                        ...
                        for(var i1 = 0; i1 < dataPoints; ++i1){
                            // Allow for a right hand Y axis so move the tickmarks to the left
                            if (properties.yaxisPosition === "right") x1 = properties.marginLeft + (properties.marginInnerLeft || 0) + i1 * ((obj.graphWidth - (properties.marginInnerLeft || 0) - (properties.marginInnerRight || 0)) / dataPoints);
                            else x1 = properties.marginLeft + (properties.marginInnerLeft || 0) + (i1 + 1) * ((obj.graphWidth - (properties.marginInnerLeft || 0) - (properties.marginInnerRight || 0)) / dataPoints);
                            RGraph.SVG.create({
                                svg: obj.svg,
                                parent: obj.svgAllGroup,
                                type: "path",
                                attr: {
                                    d: "M{1} {2} L{3} {4}".format(x1 + 0.001, startY, x1, endY),
                                    stroke: properties.xaxisColor,
                                    "stroke-width": typeof properties.xaxisLinewidth === "number" ? properties.xaxisLinewidth : 1,
                                    "shape-rendering": "crispEdges"
                                }
                    ...
                    // This style is used by line charts
                        ...
                        for(var i1 = 0; i1 < len; ++i1){
                            ...
                            if (properties.yaxisPosition === "right") {
                                x1 = properties.marginLeft + i1 * gap;
                                // If the X position is within 3 pixels of the X position of the Y
                                // axis then skip it
                                if (properties.yaxis && x1 > obj.width - properties.marginRight - 3 && x1 < obj.width - properties.marginRight + 3) continue;
                            } else x1 = properties.marginLeft + (i1 + 1) * gap;
                            // For some reason a tickmark is being drawn in the right margin
                            // so this prevents that.
                            if ((!properties.yaxisPosition || properties.yaxisPosition === "left") && x1 > obj.width - properties.marginRight) continue;
                            RGraph.SVG.create({
                                svg: obj.svg,
                                parent: obj.svgAllGroup,
                                type: "path",
        ...
        if (properties.xaxisScale) {
            if (obj.type === "scatter") {
                ...
                for(var i1 = 0; i1 < obj.xscale.labels.length; ++i1){
                    var x1 = properties.marginLeft + segment * i1 + segment + properties.xaxisLabelsOffsetx;
                    var y1 = obj.height - properties.marginBottom + (properties.xaxis ? properties.xaxisTickmarksLength + 6 : 10) + (properties.xaxisLinewidth || 1) + properties.xaxisLabelsOffsety;
                    RGraph.SVG.text({
                        object: obj,
                        parent: obj.svgAllGroup,
                        tag: "labels.xaxis",
                        text: obj.xscale.labels[i1],
                        x: x1,
                        y: y1,
                        halign: "center",
                        valign: "top",
                ...
                if (properties.xaxisLabelsCount > 0) {
                    var y1 = obj.height - properties.marginBottom + properties.xaxisLabelsOffsety + (properties.xaxis ? properties.xaxisTickmarksLength + 6 : 10), str = RGraph.SVG.numberFormat({
                        object: obj,
            ...
            // =========================================================================
                ...
                for(var i1 = 0; i1 < scale.labels.length; ++i1){
                    ...
                    if ((obj.type === "hbar" || obj.type === "scatter" && properties.xaxis) && properties.yaxisPosition === "right") x1 = obj.width - properties.marginRight - segment * i1 - segment + properties.xaxisLabelsOffsetx;
                    RGraph.SVG.text({
                        object: obj,
                        parent: obj.svgAllGroup,
                        text: obj.scale.labels[i1],
                        x: x1,
                        y: y1,
                        halign: "center",
                        valign: "top",
        ...
        } else if (typeof properties.xaxisLabels === "object" && !RGraph.SVG.isNullish(properties.xaxisLabels)) {
            ...
            if (properties.xaxisLabelsPosition === "section") {
                ...
                for(var i1 = 0; i1 < properties.xaxisLabels.length; ++i1){
                    var x1 = properties.marginLeft + (properties.marginInnerLeft || 0) + segment / 2 + i1 * segment;
                    if (obj.scale.max <= 0 && obj.scale.min < obj.scale.max) {
                        var y1 = properties.marginTop - (RGraph.SVG.ISFF ? 5 : 10) - (properties.xaxisLinewidth || 1) + properties.xaxisLabelsOffsety;
                        var valign = "bottom";
                    } else {
                        var y1 = obj.height - properties.marginBottom + (RGraph.SVG.ISFF ? 5 : 10) + (properties.xaxisLinewidth || 1) + properties.xaxisLabelsOffsety;
                        var valign = "top";
                    }
                    RGraph.SVG.text({
                        object: obj,
                        ...
                        x: x1 + properties.xaxisLabelsOffsetx,
                        y: y1,
                ...
                for(var i1 = 0; i1 < properties.xaxisLabels.length; ++i1){
                    var x1 = properties.marginLeft + i1 * segment + hmargin;
                    if (obj.scale.max <= 0 && obj.scale.min < 0) {
                        valign = "bottom";
                        y1 = properties.marginTop - (RGraph.SVG.ISFF ? 5 : 10) - (properties.xaxisTickmarksLength - 5) - (properties.xaxisLinewidth || 1) + properties.xaxisLabelsOffsety;
                    } else {
                        valign = "top";
                        y1 = obj.height - properties.marginBottom + (RGraph.SVG.ISFF ? 5 : 10) + (properties.xaxisTickmarksLength - 5) + (properties.xaxisLinewidth || 1) + properties.xaxisLabelsOffsety;
                    }
                    RGraph.SVG.text({
                        object: obj,
                        parent: obj.svgAllGroup,
        ...
        var labelsY = y1 + properties.xaxisLabelsOffsety;
        //
        // DRAW THE TITLE
        //
        if (properties.xaxisTitle) {
            ...
            var x1 = properties.marginLeft + (obj.width - properties.marginLeft - properties.marginRight) / 2 + (properties.xaxisTitleOffsetx || 0);
            var y1 = labelsY + textConf_labels.size * 1.5;
            ...
            if (typeof properties.xaxisTitleX === "number") x1 = properties.xaxisTitleX;
            if (typeof properties.xaxisTitleY === "number") y1 = properties.xaxisTitleY;
            RGraph.SVG.text({
                object: obj,
                parent: obj.svgAllGroup,
                tag: "xaxisTitle",
                text: String(properties.xaxisTitle),
                x: x1 + (properties.xaxisTitleOffsetx || 0),
                y: y1 + (properties.xaxisTitleOffsety || 0),
    ...
    RGraph.SVG.drawYAxis = function(obj) {
        ...
        if (properties.yaxis) {
            ...
            if (obj.type === "hbar") {
                var x1 = obj.getXCoord(properties.xaxisScaleMin > 0 ? properties.xaxisScaleMin : 0);
                if (properties.xaxisScaleMin < 0 && properties.xaxisScaleMax <= 0) x1 = obj.getXCoord(properties.xaxisScaleMax);
            } else if (properties.yaxisPosition === "right") var x1 = obj.width - properties.marginRight;
            else var x1 = properties.marginLeft;
            var axis = RGraph.SVG.create({
                svg: obj.svg,
                parent: obj.svgAllGroup,
                type: "path",
                attr: {
                    d: "M{1} {2} L{3} {4}".format(x1, properties.marginTop, x1, obj.height - properties.marginBottom),
            ...
            if (obj.type === "hbar") {
                ...
                if (properties.yaxisTickmarks) {
                    for(var i1 = 0; i1 < (len || properties.yaxisTickmarksCount); ++i1){
                        ...
                        var tick = RGraph.SVG.create({
                            ...
                            type: "path",
                            attr: {
                                d: "M{1} {2} L{3} {4}".format(startX, y1, endX, y1 + 0.001),
                                stroke: properties.yaxisColor,
                                "stroke-width": typeof properties.yaxisLinewidth === "number" ? properties.yaxisLinewidth : 1,
                                "shape-rendering": "crispEdges"
                            }
            ...
            //
                ...
                var height = obj.graphHeight / properties.yaxisLabelsCount, y1 = properties.marginTop, len = properties.yaxisLabelsCount, startX = properties.marginLeft, endX = properties.marginLeft - properties.yaxisTickmarksLength;
                ...
                if (properties.yaxisTickmarks) {
                    for(var i1 = 0; i1 < len; ++i1){
                        ...
                        // Egde case
                        if (!(obj.max <= 0 && obj.min < obj.max && y1 === obj.properties.marginTop) && !(obj.min < 0 && obj.max > 0 && y1 <= obj.getYCoord(0) + 1 && y1 >= obj.getYCoord(0) - 1)) // Avoid a tickmark at zero                        
                        ...
                        var axis = RGraph.SVG.create({
                            ...
                            type: "path",
                            attr: {
                                d: "M{1} {2} L{3} {4}".format(startX, y1, endX, y1),
                                stroke: properties.yaxisColor,
                                "stroke-width": typeof properties.yaxisLinewidth === "number" ? properties.yaxisLinewidth : 1,
                                "shape-rendering": "crispEdges"
                            }
        ...
        if (properties.yaxisScale) {
            ...
            for(var i1 = 0; i1 < obj.scale.labels.length; ++i1){
                var y1 = obj.height - properties.marginBottom - segment * i1 - segment;
                RGraph.SVG.text({
                    object: obj,
                    parent: obj.svgAllGroup,
                    tag: "labels.yaxis",
                    ...
                    y: y1 + properties.yaxisLabelsOffsety,
            ...
            //
            var y1 = obj.height - properties.marginBottom, // Changed this to use obj.scale.min instead of properties.yaxisScaleMin
        ...
        //
        } else if (properties.yaxisLabels && properties.yaxisLabels.length) for(var i1 = 0; i1 < properties.yaxisLabels.length; ++i1){
            var segment = (obj.graphHeight - (properties.marginInnerTop || 0) - (properties.marginInnerBottom || 0)) / properties.yaxisLabels.length, y1 = properties.marginTop + (properties.marginInnerTop || 0) + segment * i1 + segment / 2 + properties.yaxisLabelsOffsety, x1 = properties.marginLeft - 7 - (properties.yaxisLinewidth || 1) + properties.yaxisLabelsOffsetx, halign = "right";
            if (properties.yaxisPosition === "right") {
                halign = "left";
                x1 = obj.width - properties.marginRight + 7 + (properties.yaxisLinewidth || 1) + properties.yaxisLabelsOffsetx;
            }
            // HBar labels
            if (obj.type === "hbar" && (obj.scale.min < obj.scale.max && obj.scale.max <= 0 || properties.yaxisPosition === "right")) {
                halign = "left";
                x1 = obj.width - properties.marginRight + 7 + properties.yaxisLabelsOffsetx;
            // HBar labels (again?)
                ...
                y1 = properties.marginTop + (properties.marginInnerTop || 0) + segment * i1 + segment / 2 + properties.yaxisLabelsOffsety;
            // Specific scale
            } else {
                var segment = (obj.graphHeight - (properties.marginInnerTop || 0) - (properties.marginInnerBottom || 0)) / (properties.yaxisLabels.length - 1);
                y1 = obj.height - properties.marginBottom - segment * i1 + properties.yaxisLabelsOffsety;
            }
            var text = RGraph.SVG.text({
                object: obj,
                parent: obj.svgAllGroup,
                tag: "labels.yaxis",
                text: properties.yaxisLabels[i1] ? properties.yaxisLabels[i1] : "",
                x: x1,
                y: y1,
                halign: halign,
                valign: "center",
        ...
        if (properties.yaxisTitle) {
            ...
            var x1 = properties.yaxisPosition === "right" ? obj.width - properties.marginRight + 5 + maxLabelLength + 10 : properties.marginLeft - 5 - maxLabelLength - 10;
            var y1 = (obj.height - properties.marginTop - properties.marginBottom) / 2 + properties.marginTop;
            // Specific X and Y coordinates for the title
            if (typeof properties.yaxisTitleOffsetx === "number") x1 += properties.yaxisTitleOffsetx;
            if (typeof properties.yaxisTitleOffsety === "number") y1 += properties.yaxisTitleOffsety;
            // Specific X and Y coordinates for the title
            if (typeof properties.yaxisTitleX === "number") x1 = properties.yaxisTitleX;
            if (typeof properties.yaxisTitleY === "number") y1 = properties.yaxisTitleY;
    ...
    RGraph.SVG.extractLink = function(str) {
        if (str.match(/<a /) && str.match(/href="(.+?)"/i)) {
            ...
            // and if so use it.
            if (str.match(/target="(.*?)"/)) var target1 = RegExp.$1;
            else var target1 = "_self";
        } else {
            var text = str;
            var anchor = null;
        }
    ...
    RGraph.SVG.drawBackground = function(obj) {
        ...
        if (properties.backgroundGrid) {
            ...
            if (properties.backgroundGridHlines) {
                if (typeof properties.backgroundGridHlinesCount === "number") var count = properties.backgroundGridHlinesCount;
                    ...
                    if (typeof properties.yaxisLabels === "object" && !RGraph.SVG.isNullish(properties.yaxisLabels) && properties.yaxisLabels.length) var count = properties.yaxisLabels.length;
                ...
                for(var i1 = 0; i1 <= count; ++i1)parts.push("M{1} {2} L{3} {4}".format(properties.marginLeft + properties.variant3dOffsetx, properties.marginTop + obj.graphHeight / count * i1 - properties.variant3dOffsety, obj.width - properties.marginRight + properties.variant3dOffsetx, properties.marginTop + obj.graphHeight / count * i1 - properties.variant3dOffsety));
            ...
            if (properties.backgroundGridVlines) {
                ...
                for(var i1 = 0; i1 <= count; ++i1)parts.push("M{1} {2} L{3} {4}".format(properties.marginLeft + obj.graphWidth / count * i1 + properties.variant3dOffsetx, properties.marginTop - properties.variant3dOffsety, properties.marginLeft + obj.graphWidth / count * i1 + properties.variant3dOffsetx, obj.height - properties.marginBottom - properties.variant3dOffsety));
            }
        ...
        if (properties.backgroundBorder) {
            ...
            if (properties.backgroundBorderDashed) dasharray = "3 5";
    ...
    RGraph.SVG.getScale = function(opt) {
        ...
        if (max === 0 && min === 0) {
            ...
            for(var i1 = 0; i1 < numlabels; ++i1){
                var label = ((max - min) / numlabels * (i1 + 1) + min).toFixed(decimals);
        ...
        //
            ...
            for(var i1 = 0; i1 < numlabels; ++i1){
                var label = RGraph.SVG.numberFormat({
                    object: obj,
                    num: ((i1 + 1) / numlabels * (topValue - min) + min).toFixed(decimals),
                    prepend: unitsPre,
                    append: unitsPost,
                    point: point,
                    thousand: thousand,
            ...
            for(var i1 = 0; i1 < numlabels; ++i1){
                scale.labels.push(RGraph.SVG.numberFormat({
                    object: obj,
                    formatter: formatter,
                    num: ((i1 + 1) / numlabels * (max - min) + min).toFixed(decimals),
                    prepend: unitsPre,
                    append: unitsPost,
                    point: point,
                    thousand: thousand
    ...
    RGraph.SVG.arraySum = function(arr) {
        ...
        var i1, sum, len = arr.length;
        for(i1 = 0, sum = 0; i1 < len; sum += arr[i1++]);
        return sum;
    };
    //
    ...
    RGraph.SVG.arrayMax = function(arr) {
        ...
        if (typeof arr === "number") return arr;
        if (RGraph.SVG.isNullish(arr)) return 0;
        for(var i1 = 0, len = arr.length; i1 < len; ++i1)if (typeof arr[i1] === "number") {
            var val = arguments[1] ? Math.abs(arr[i1]) : arr[i1];
    ...
    RGraph.SVG.arrayFill = RGraph.SVG.arrayPad = function(args) {
        ...
        if (arr.length < len) {
            ...
            for(var i1 = arr.length; i1 < len; i1 += 1)arr[i1] = val;
        }
        return arr;
    };
    //
    ...
    RGraph.SVG.arrayLinearize = function() {
        ...
        for(var i1 = 0, len = args.length; i1 < len; ++i1){
            if (typeof args[i1] === "object" && args[i1]) for(var j1 = 0, len2 = args[i1].length; j1 < len2; ++j1){
                var sub = RGraph.SVG.arrayLinearize(args[i1][j1]);
    ...
    RGraph.SVG.arrayShift = function(arr) {
        var ret = [];
        for(var i1 = 1, len = arr.length; i1 < len; ++i1)ret.push(arr[i1]);
        return ret;
    };
    //
    ...
    RGraph.SVG.arrayReverse = function(arr) {
        if (!arr) return;
        var newarr = [];
        for(var i1 = arr.length - 1; i1 >= 0; i1 -= 1)newarr.push(arr[i1]);
        return newarr;
    };
    //
        ...
        switch(typeof array){
            ...
            case "object":
                if (array.constructor.toString().indexOf("Array") >= 0) {
                    ret = new Array();
                    for(var i1 = 0; i1 < array.length; ++i1)ret[i1] = RGraph.SVG.arrayClone(array[i1], objects, maxdepth - 1);
                } else if (array.constructor.toString().indexOf("Object") > 0 && objects) {
                    ret = new Object();
                    for(var i1 in array)ret[i1] = RGraph.SVG.arrayClone(array[i1], true, maxdepth - 1);
                }
                break;
            case "function":
                ret = array;
    ...
    RGraph.SVG.arrayInvert = function(arr) {
        for(var i1 = 0, len = arr.length; i1 < len; ++i1)arr[i1] = !arr[i1];
        return arr;
    };
    //
    ...
    RGraph.SVG.arrayTrim = function(arr) {
        ...
        for(var i1 = 0; i1 < arr.length; i1++){
            if (arr[i1]) content = true;
            if (content) out.push(arr[i1]);
        }
    ...
    RGraph.SVG.text = function(opt) {
        ...
        for(var i1 in RGraph.SVG.text.defaults)if (typeof i1 === "string" && typeof opt[i1] === "undefined") opt[i1] = RGraph.SVG.text.defaults[i1];
        var obj = opt.object, parent = opt.parent || opt.object.svgAllGroup, size = typeof opt.size === "number" ? opt.size + "pt" : (typeof opt.size === "string" ? opt.size.replace(/pt$/, "") : 12) + "pt", bold = opt.bold ? "bold" : "normal", font = opt.font ? opt.font : "sans-serif", italic = opt.italic ? "italic" : "normal", halign = opt.halign, valign = opt.valign, str = opt.text, x1 = opt.x, y1 = opt.y, color = opt.color ? opt.color : opt.link ? "blue" : "black", background = opt.background || null, backgroundRounded = opt.backgroundRounded || 0, padding = opt.padding || 0, link = opt.link || "", linkTarget = opt.linkTarget || "_blank", events = opt.events === true || opt.link ? true : false, angle = opt.angle;
        //
        ...
        if (link) {
            ...
            if (RGraph.SVG.isString(opt.object.properties.textLinkColor)) color = opt.object.properties.textLinkColor;
            else color = "blue";
        } else if (str.match(/<a /)) {
        ...
        //
            ...
            // up as required
            //
            if (valign === "bottom") y1 -= str.length * lineHeight;
            if (valign === "center" || valign === "middle") y1 -= str.length * lineHeight / 2;
            var text = RGraph.SVG.create({
                svg: obj.svg,
                parent: link ? a : opt.parent,
                type: "text",
            ...
            for(var i1 = 0; i1 < str.length; ++i1){
                var tspan = RGraph.SVG.create({
                    ...
                    attr: {
                        x: x1,
                        dy: dimensions ? dimensions[1] * (i1 ? 1 : 0) + 3 : 0
                    }
                });
                var textNode = document.createTextNode(str[i1]);
        ...
        if (typeof angle === "number" && angle && text) {
            ...
            text.setAttribute("transform", "translate({1} {2}) rotate({3})".format(x1, y1, -1 * angle));
        }
        //
        ...
        if (RGraph.SVG.ISIE && valign === "hanging" && text) text.setAttribute("y", y1 + text.scrollHeight / 2);
        else if (RGraph.SVG.ISIE && valign === "middle" && text) text.setAttribute("y", y1 + text.scrollHeight / 3);
        if (RGraph.SVG.ISFF && text) Y = y1 + text.scrollHeight / 3;
        return text;
    };
    ...
    RGraph.SVG.text.find = function(opt) {
        ...
        var found = [];
        for(var i1 = 0, len = nodes.length; i1 < len; ++i1){
            var text = false, tag = false;
            // Exact match or regex on the text
            if (typeof opt.text === "string" && nodes[i1].innerHTML === opt.text) text = true;
            else if (typeof opt.text === "object" && nodes[i1].innerHTML.match(opt.text)) text = true;
            else if (typeof opt.text === "undefined") text = true;
            // Exact match or regex on the tag
            if (typeof opt.tag === "string" && nodes[i1].getAttribute("tag") === opt.tag) tag = true;
            else if (typeof opt.tag === "object" && nodes[i1].getAttribute("tag").match(opt.tag)) tag = true;
            else if (typeof opt.tag === "undefined") tag = true;
            // Did all of the conditions pass?
            if (text === true && tag === true) found.push(nodes[i1]);
        }
    ...
    RGraph.SVG.createUID = function() {
        return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(c) {
            var r1 = Math.random() * 16 | 0, v1 = c == "x" ? r1 : r1 & 0x3 | 0x8;
    ...
    RGraph.SVG.isFixed = function(svg) {
        var obj = svg.parentNode, i1 = 0;
        while(obj && obj.tagName.toLowerCase() != "body" && i1 < 99){
    ...
    RGraph.SVG.setShadow = function(options) {
        var obj = options.object, id = options.id, offsetx = options.offsetx || 0, offsety = options.offsety || 0, blur = options.blur || 0, color = options.color || "gray";
    ...
    RGraph.SVG.groupedIndexToSequential = function(opt) {
        ...
        for(var i1 = 0, seq = 0; i1 <= dataset; ++i1)for(var j1 = 0; j1 < obj.data[dataset].length; ++j1){
            if (i1 === dataset && j1 === index) return seq;
            seq++;
        }
        return seq;
    };
    ...
    RGraph.SVG.TRIG.getAngleByXY = function(opt) {
        var cx = opt.cx, cy = opt.cy, x1 = opt.x, y1 = opt.y;
        var angle = Math.atan((y1 - cy) / (x1 - cx));
        if (x1 >= cx && y1 >= cy) angle += RGraph.SVG.TRIG.HALFPI;
        else if (x1 >= cx && y1 < cy) angle = angle + RGraph.SVG.TRIG.HALFPI;
        else if (x1 < cx && y1 < cy) angle = angle + RGraph.SVG.TRIG.PI + RGraph.SVG.TRIG.HALFPI;
    ...
    RGraph.SVG.TRIG.getRadiusEndPoint = function(opt) {
        ...
        if (arguments.length === 1) {
            ...
            var angle = opt.angle, r1 = opt.r;
        } else if (arguments.length === 4) var angle = arguments[0], r1 = arguments[1];
    ...
    RGraph.SVG.TRIG.drawSegment = function(opt) {
        ...
        var element = RGraph.SVG.create({
            ...
            attr: {
                ...attr,
                d: "M {1} {2} {3} z".format(opt.cx, opt.cy, path)
            },
            style: {
                ...style
            }
    ...
    RGraph.SVG.drawTitle = function(obj) {
        ...
        var x1 = (obj.width - properties.marginLeft - properties.marginRight) / 2 + properties.marginLeft, y1 = properties.marginTop - 10, valign = "bottom";
        // If theres key defined then move the title up
        if (!RGraph.SVG.isNullish(obj.properties.key)) y1 -= 20;
        ...
        if (typeof obj.properties.yaxisScaleMax === "number" && obj.properties.yaxisScaleMax <= 0 && obj.properties.yaxisScaleMin < obj.properties.yaxisScaleMax) {
            valign = "top";
            y1 = obj.height - obj.properties.marginBottom + 10;
        }
        // Custom X coordinate
        if (typeof properties.titleX === "number") x1 = properties.titleX;
        // Custom Y coordinate
        if (typeof properties.titleY === "number") y1 = properties.titleY;
        // Custom X coordinate
        if (typeof properties.titleOffsetx === "number") x1 += properties.titleOffsetx;
        // Custom Y coordinate
        if (typeof properties.titleOffsety === "number") y1 += properties.titleOffsety;
    ...
    RGraph.SVG.parseColorLinear = function(opt) {
        ...
        if (color.match(/^gradient\((.*)\)$/i)) {
            ...
            for(var j1 = 1, len = parts.length; j1 < len; ++j1)RGraph.SVG.create({
                type: "stop",
                parent: grad,
                attr: {
                    offset: j1 * diff * 100 + "%",
                    "stop-color": RGraph.SVG.trim(parts[j1])
                }
            });
        }
    ...
    RGraph.SVG.resetColorsToOriginalValues = function(opt) {
        ...
        if (obj.originalColors) {
            ...
            for(var j1 in obj.originalColors)if (typeof j1 === "string") obj.properties[j1] = RGraph.SVG.arrayClone(obj.originalColors[j1]);
        }
        //
    ...
    RGraph.SVG.clear = function() {
        ...
        for(var i1 = 1; i1 <= 100; ++i1){
            if (svg && svg["background" + i1]) // Clear all the nodes within this group
            svg["background" + i1].replaceChildren();
            else break;
        }
        const groups = svg.getElementsByClassName("all-elements");
        for(let i1 = 0; i1 < groups.length; ++i1)groups[i1].replaceChildren();
        //
        // Add a background color if requested
        //
        // TODO This needs more testing to see if it works
        //      satisfactorily
        if (color) RGraph.SVG.create({
            svg: svg,
            type: "rect",
            attr: {
                x: -10,
    ...
    RGraph.SVG.fireCustomEvent = function(obj, name) {
        if (obj && obj.isRGraph) {
            if (name.substr(0, 2) !== "on") name = "on" + name;
            if (typeof obj.uid === "string" && typeof RGraph.SVG.events === "object" && typeof RGraph.SVG.events[obj.uid] === "object" && RGraph.SVG.events[obj.uid].length > 0) {
                for(var j1 = 0, len = RGraph.SVG.events[obj.uid].length; j1 < len; ++j1)if (RGraph.SVG.events[obj.uid][j1] && RGraph.SVG.events[obj.uid][j1].event === name) RGraph.SVG.events[obj.uid][j1].func(obj);
            }
        }
    };
    //
    ...
    //
    RGraph.SVG.removeCustomEventListener = function(obj, i1) {
        if (typeof RGraph.SVG.events === "object" && typeof RGraph.SVG.events[obj.uid] === "object" && typeof RGraph.SVG.events[obj.uid][i1] === "object") RGraph.SVG.events[obj.uid][i1] = null;
    };
    //
    ...
    RGraph.SVG.removeHighlight = function(obj) {
        ...
        if (highlight && RGraph.SVG.isArray(highlight) && highlight.length) {
            for(var i1 = 0, len = highlight.length; i1 < len; ++i1)if (highlight[i1].parentNode) //obj.svg.removeChild(highlight[i]);
            highlight[i1].parentNode.removeChild(highlight[i1]);
    ...
    RGraph.SVG.redraw = function() {
        if (arguments.length === 1) {
            ...
            for(var i1 in tags)RGraph.SVG.redraw(tags[i1]);
        }
    };
    //
    ...
    RGraph.SVG.OR.get = function() {
        ...
        if (typeof arguments[0] === "string" && arguments[0].substr(0, 3).toLowerCase() === "id:") {
            var ret = [];
            for(var i1 = 0; i1 < RGraph.SVG.OR.objects.length; ++i1)if (RGraph.SVG.OR.objects[i1] && RGraph.SVG.OR.objects[i1].id === arguments[0].substr(3)) ret.push(RGraph.SVG.OR.objects[i1]);
            return ret;
        }
        ...
        if (typeof arguments[0] === "string" && arguments[0].substr(0, 4).toLowerCase() === "type") {
            var ret = [];
            for(var i1 = 0; i1 < RGraph.SVG.OR.objects.length; ++i1)if (RGraph.SVG.OR.objects[i1].type === arguments[0].substr(5)) ret.push(RGraph.SVG.OR.objects[i1]);
            return ret;
        }
        // Fetch an object that has a specific UID
        if (typeof arguments[0] === "string" && arguments[0].substr(0, 3).toLowerCase() === "uid") {
            var ret = [];
            for(var i1 = 0; i1 < RGraph.SVG.OR.objects.length; ++i1)if (RGraph.SVG.OR.objects[i1].uid === arguments[0].substr(4)) ret.push(RGraph.SVG.OR.objects[i1]);
            return ret;
        }
    ...
    RGraph.SVG.OR.tags = function() {
        var tags = [];
        for(var i1 = 0; i1 < RGraph.SVG.OR.objects.length; ++i1)if (RGraph.SVG.OR.objects[i1] && !tags[RGraph.SVG.OR.objects[i1].svg.parentNode.id]) tags[RGraph.SVG.OR.objects[i1].svg.parentNode.id] = RGraph.SVG.OR.objects[i1].svg;
        return tags;
    };
    //
    ...
    RGraph.SVG.getSVGXY = function(svg) {
        var x1 = 0, y1 = 0, el = svg.parentNode; // !!!
...
*/ };
    ...
    RGraph.SVG.stringsToNumbers = function(str) {
        ...
        if (typeof str === "object" && !RGraph.SVG.isNullish(str)) for(var i1 = 0, len = str.length; i1 < len; i1 += 1)str[i1] = RGraph.SVG.stringsToNumbers(str[i1], sep);
        return str;
    };
    ...
    RGraph.SVG.random = function(opt) {
        if (RGraph.SVG.isObject(opt)) var min = opt.min, max = opt.max, dp = opt.dp || opt.decimals || 0;
        else if (RGraph.SVG.isNumber(opt)) var min = opt, max = arguments[1], dp = arguments[2] || 0;
        var r1 = Math.random();
        return Number(((max - min) * r1 + min).toFixed(dp));
    };
    //
    ...
    RGraph.SVG.arrayRandom = function(opt) {
        ...
        for(var i1 = 0, arr = []; i1 < num; i1 += 1)arr.push(RGraph.SVG.random({
            min: min,
            max: max,
            dp: dp
        }));
    ...
    RGraph.SVG.getGlobals = function(obj) {
        var properties = obj.properties;
        for(var i1 in RGraph.SVG.GLOBALS)if (typeof i1 === "string") obj.set(i1, RGraph.SVG.arrayClone(RGraph.SVG.GLOBALS[i1]));
    };
    //
    ...
    RGraph.SVG.getErrorbarsMaxValue = function(opt) {
        ...
        if (typeof properties.errorbars === "object" && !RGraph.SVG.isNullish(properties.errorbars) && typeof properties.errorbars[index] === "number") var value = properties.errorbars[index];
        else if (typeof properties.errorbars === "object" && !RGraph.SVG.isNullish(properties.errorbars) && typeof properties.errorbars[index] === "object" && !RGraph.SVG.isNullish(properties.errorbars[index]) && typeof properties.errorbars[index].max === "number") var value = properties.errorbars[index].max;
        else var value = 0;
        return value;
    };
    ...
    RGraph.SVG.getErrorbarsMinValue = function(opt) {
        var obj = opt.object, properties = obj.properties, index = opt.index;
        if (typeof properties.errorbars === "object" && !RGraph.SVG.isNullish(properties.errorbars) && typeof properties.errorbars[index] === "object" && !RGraph.SVG.isNullish(properties.errorbars[index]) && typeof properties.errorbars[index].min === "number") var value = properties.errorbars[index].min;
        else var value = null;
        return value;
    };
    ...
    RGraph.SVG.getErrorbarsColor = function(opt) {
        ...
        if (typeof properties.errorbars === "object" && !RGraph.SVG.isNullish(properties.errorbars) && typeof properties.errorbars[index] === "object" && !RGraph.SVG.isNullish(properties.errorbars[index]) && typeof properties.errorbars[index].color === "string") color = properties.errorbars[index].color;
        return color;
    };
    ...
    RGraph.SVG.responsive = function(conf) {
        ...
        for(var i1 = 0; i1 < conf.length; ++i1){
            ...
            conf[i1].minWidth = RGraph.SVG.isNullish(conf[i1].minWidth) ? 0 : conf[i1].minWidth;
            conf[i1].maxWidth = RGraph.SVG.isNullish(conf[i1].maxWidth) ? 100000 : conf[i1].maxWidth;
        ...
        function matchFunction(rule) {
            ...
            if (typeof rule.options === "object") {
                for(var j1 in rule.options)if (typeof j1 === "string") {
                    obj.set(j1, rule.options[j1]);
                    // Set the original colors to the new colors
                    // if necessary
                    if (j1 === "colors" && obj.originalColors) obj.originalColors = RGraph.SVG.arrayClone(rule.options[j1]);
                }
            }
            //
            ...
            if (typeof rule.css === "object") {
                for(var j1 in rule.css)if (typeof j1 === "string") setCSS(obj.svg.parentNode, j1, rule.css[j1]);
            }
            //
            // Are there any CSS properties to set on the SVGs PARENT tag?
            //
            if (typeof rule.parentCss === "object") {
                for(var j1 in rule.parentCss)if (typeof j1 === "string") setCSS(obj.svg.parentNode.parentNode, j1, rule.parentCss[j1]);
            }
    ...
    RGraph.SVG.responsiveOld = function(conf) {
        var opt = arguments[1] || {}, // This function is added to each object in their constructors so the this
        ...
        var func = function() {
            ...
            //
            if (!matched && (document.documentElement.clientWidth <= conf[i1].maxWidth || RGraph.SVG.isNullish(conf[i1].maxWidth))) {
                matched = true;
                // If a width is defined for this rule set it
                if (typeof conf[i1].width === "number") {
                    obj.svg.setAttribute("width", conf[i1].width);
                    obj.container.style.width = conf[i1].width + "px";
                }
                //
                // If a height is defined for this rule set it
                //
                if (typeof conf[i1].height === "number") {
                    obj.svg.setAttribute("height", conf[i1].height);
                    obj.container.style.height = conf[i1].height + "px";
                }
                //
                // Are there any options to be set?
                //
                ...
                if (typeof conf[i1].css === "object") {
                    for(var j1 in conf[i1].css)if (typeof j1 === "string") setCSS(obj.svg.parentNode, j1, conf[i1].css[j1]);
                }
                //
                // Are there any CSS properties to set on the SVGs PARENT tag?
                //
                if (typeof conf[i1].parentCss === "object") {
                    for(var j1 in conf[i1].parentCss)if (typeof j1 === "string") setCSS(obj.svg.parentNode.parentNode, j1, conf[i1].parentCss[j1]);
                }
                // Redraw the chart with SVG this is done by the redraw() function
                RGraph.SVG.redraw();
                // Run the callback function if it's defined
    ...
    RGraph.SVG.labelSubstitution = function(args) {
        ...
        while(text.match(regexp)){
            ...
            for(var i1 = 0, len = str.length; i1 < len; ++i1)str = str.replace(/\r?\n/, "\\n");
    ...
    RGraph.SVG.addCustomText = function(obj) {
        if (RGraph.SVG.isArray(obj.properties.text) && obj.properties.text.length) for(var i1 = 0; i1 < obj.properties.text.length; ++i1){
    ...
    //
    RGraph.SVG.GET.text = RGraph.SVG.GET.string = function(key) {
    ...
    // @param string key The part to get
    //
    RGraph.SVG.GET.json = RGraph.SVG.GET.object = function(key) {
        var parts = RGraph.SVG.GET.parse();
        if (!parts[key]) return null;
    ...
    //                        comma)
    //
    RGraph.SVG.GET.list = RGraph.SVG.GET.array = function(key) {
        var parts = RGraph.SVG.GET.parse();
        if (!parts[key]) return null;
    ...
    RGraph.SVG.removeHighlight = function() {
        ...
        if (RGraph.SVG.isArray(highlight)) {
            for(var i1 = 0; i1 < highlight.length; ++i1)if (highlight[i1] && highlight[i1].parentNode) highlight[i1].parentNode.removeChild(highlight[i1]);
        } else if (highlight) {
    ...
    RGraph.SVG.clipTo = function(obj, dimensions) {
        ...
        if (!RGraph.SVG.clipTo.counter) RGraph.SVG.clipTo.counter = 1;
        else RGraph.SVG.clipTo.counter++;
        ...
        if (typeof dimensions === "string") {
            if (dimensions === "tophalf") {
                ...
                obj.svgAllGroup.setAttribute("clip-path", "url(#" + id + ")");
    ...
    RGraph.SVG.drawHorizontalLines = function(obj) {
        var lines = obj.properties.horizontalLines, avg, x1, y1, label, halign, valign, hmargin = 10, vmargin = 5, position, textFont, textSize, textColor, textBold, textItalic, data, linewidth;
        if (lines) {
            //
                ...
                switch(obj.type){
                    ...
                    case "bar":
                        ...
                        if (typeof conf.value === "number") y1 = obj.getYCoord(conf.value);
                            ...
                            var total = 0;
                            obj.data.map((v1)=>{
                                if (RGraph.SVG.isNumber(v1)) total += v1;
                                else if (RGraph.SVG.isArray(v1)) total += RGraph.SVG.arraySum(v1);
                            });
                            var num = 0;
                            for(let i1 = 0; i1 < obj.data.length; ++i1){
                                if (obj.properties.grouping === "grouped") num += RGraph.SVG.isArray(obj.data[i1]) ? obj.data[i1].length : 1;
                    ...
                    case "scatter":
                        ...
                        if (typeof conf.value === "number") y1 = obj.getYCoord(conf.value);
                            ...
                            var sum = 0;
                            obj.data[0].forEach((v1)=>sum += v1.y);
    ...
    RGraph.SVG.roundRect = function(options) {
        ...
        if (RGraph.SVG.isNumber(options.radius) || RGraph.SVG.isArray(options.radius) && options.radius.length < 4 && RGraph.SVG.isNumber(options.radius[0])) {
            if (RGraph.SVG.isArray(options.radius)) options.radius = options.radius[0];
        ...
        //
        var path = "M {1} {2} ".format(options.x + options.width / 2, options.y);
    ...
    RGraph.SVG.installClipping = function(obj) {
        ...
        var clipPath = obj.create('<clipPath id="{1}">'.format(id), obj.svg.defs);
        if (RGraph.SVG.isString(obj.properties.clip)) {
            ...
            // HORIZONTAL PERCENTAGES
                ...
                from = Number(from);
                to = Number(to);
                var width = obj.width - obj.properties.marginLeft - obj.properties.marginRight, x1 = from / 100 * width + obj.properties.marginLeft, y1 = 0, width = (to - from) / 100 * (obj.width - obj.properties.marginLeft - obj.properties.marginRight), height = obj.height;
                var clipPathRect = obj.create('<rect x="{1}" y="{2}" width="{3}" height="{4}">'.format(x1, y1, width, height), clipPath);
            // VERTICAL PERCENTAGES
                ...
                from = Number(from);
                to = Number(to);
                var x1 = 0, width = obj.width, //y1     = ((to - from) / 100) * (obj.height - obj.properties.marginTop - obj.properties.marginBottom),
                y2 = to / 100 * (obj.height - obj.properties.marginTop - obj.properties.marginBottom);
                y1 = obj.height - obj.properties.marginBottom - y2, height = (obj.height - obj.properties.marginTop - obj.properties.marginBottom) * ((to - from) / 100);
                var clipPathRect = obj.create('<rect x="{1}" y="{2}" width="{3}" height="{4}">'.format(x1, y1, width, height), clipPath);
            // RADIAL PERCENTAGES
                ...
                // object
                if (!RGraph.SVG.isNumber(obj.centerx) || !RGraph.SVG.isNumber(obj.centery) || !RGraph.SVG.isNumber(obj.radius)) alert("[RGRAPH CLIPPING] To use the r: syntax the object (Type: {1}, ID: {2}, UID: {3}) must support the centerx, centery and radius properties.".format(obj.type, obj.id, obj.uid));
                var centerx = obj.centerx, centery = obj.centery, r1 = from / 100 * obj.radius, r2 = to / 100 * obj.radius;
            ...
            //
                ...
                var centerx = RegExp.$1, centery = RegExp.$2, radius = RegExp.$3, start = RegExp.$4, end = RegExp.$5;
            ...
            //
                ...
                var centerx = RegExp.$1, centery = RegExp.$2, radius = RegExp.$3;
                // Add the path to the all group
            ...
            //
                ...
                if (obj.clipToScaleWorker) obj.clipToScaleWorker(clipPath);
                else console.log("The scale: clipping option isn't implemented for this chart type (" + obj.type + ")");
            } else // An SVG path string
            var path = RGraph.SVG.create({
                svg: obj.svg,
                type: "path",
                parent: clipPath,
        ...
        } else if (RGraph.SVG.isArray(obj.properties.clip) && obj.properties.clip.length === 4 && RGraph.SVG.isNumber(obj.properties.clip[0]) && RGraph.SVG.isNumber(obj.properties.clip[1]) && RGraph.SVG.isNumber(obj.properties.clip[2]) && RGraph.SVG.isNumber(obj.properties.clip[3])) var clipPathRect = obj.create('<rect x="{1}" y="{2}" width="{3}" height="{4}">'.format(obj.properties.clip[0], obj.properties.clip[1], obj.properties.clip[2], obj.properties.clip[3]), clipPath);
        else if (RGraph.SVG.isArray(obj.properties.clip) && RGraph.SVG.isArray(obj.properties.clip[0]) && obj.properties.clip[0].length === 2) {
...
window.$p = function(obj) {
    ...
    switch(typeof obj){
        ...
        case "object":
            // In case of null
            if (RGraph.SVG.isNullish(obj)) str += indent + "null\n";
            else {
                str += indent + "Object {" + "\n";
...
RGraph.SVG.isTextual = function(obj) {
    return typeof obj === "string" || typeof obj === "number";
};
RGraph.SVG.isNumeric = function(value) {
    value = String(value);
...
RGraph.SVG.isBool = RGraph.SVG.isBoolean = function(obj) {
...
//# sourceMappingURL=index.62b14eef.js.map
